...
- **Ticket / manifest / build memory** — `docs/tickets/` is the committed contract backlog;
  `docs/build/` is durable build memory; a **gate** (`HG-nn`) is a human decision a ticket may pause on.
...
- Never commit anything under `.agents/scratch/` (gitignored agent scratch) or `sbom.cdx.json`.
