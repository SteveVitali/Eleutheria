...
| `api/src/api/store_pg.py` | ~1170 | `PgReadStore` — read-only view over the PG spine, on a liveness-checked pool (ADR-108) |
...
4. **`PgReadStore` reads run on a pooled connection (ADR-108).** Every public read method is wrapped in
   `@_pooled`, which checks out one connection for the call (re-applying `SET ROLE`, liveness-checked) and retries
   once on a dropped connection. `self._conn` is only valid inside such a call; a new public read method needs
