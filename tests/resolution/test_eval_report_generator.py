...
def test_committed_report_carries_the_hosted_scale_measurement(gen) -> None:
    """P30.2: the Scale section is the committed hosted measurement, never a fabricated
    number, and the PROVISIONAL-eval disclosure is retained (D-R6.1-EVAL stays OPEN)."""
    ...
    assert f"- resolved sites / decisions: {kwargs['resolved_site_count']}" in committed
    assert f"- resolution envelopes materialized: {kwargs['resolutions_materialized']}" in committed
    assert f"- dedup ratio: {kwargs['dedup_ratio']:.3f}" in committed
