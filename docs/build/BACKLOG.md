...
- **BL-032** — Per-source rights review + registry completion (flip ingestion_permitted) · _rights/legal_ · L · status=closed · gate HG-03
  - sources: RISK-P0-19 LD-P05 ADR-063
- **BL-033** — Stage-0 outreach / Eyes-on-Flock archival succession recorded · _rights/legal_ · M · status=closed · gate HG-04
  - sources: RISK-P0-20 LD-P01

## P21.2

- **BL-004** — Persist reconcile/inference/tasks/contradiction layer to PG (compute-on-read -> materialised) · _deferred-feature_ · L · status=closed · gate accepted:compute-on-read A5/HG-14
...
- **BL-007** — Wire web surfaces to the live /v1 API/exports (replace committed TS fixtures) · _deferred-feature_ · L · status=closed
  - sources: RISK-P15-07 RISK-P15-08 RISK-P15-13 RISK-P15-14 RISK-P15-20 RISK-P15-25 RISK-P15-26 RISK-P15-31 RISK-P15-32 LD-V08 RISK-P2-14 RISK-P9-08 RISK-P13-08 RISK-P13-09 RISK-P13-13 ADR-049 ADR-050 ADR-052 ADR-053 ADR-032 ADR-046 ADR-066
...
- **BL-029** — Zenodo live deposit + object store (rendered tiles + dry-run/push-stub DELIVERED P21.5 ADR-067; LIVE deposit+store gate-pending HG-07) · _external-dep_ · L · status=open · gate HG-07
  - sources: RISK-P14-16 LD-V07 ADR-048 RISK-P14-17 LD-F07 LD-H08 LD-D09 ADR-015 ADR-067
- **BL-030** — Zero-cost / degraded-but-alive keepalive tested (SIG-GOV-021) · _operational-prereq_ · M · status=closed
  - sources: RISK-P0-12 LD-P07

## P21.6

- **BL-009** — Curation/review web UI shipped (P21.6/ADR-068); asset-promotion gate SPLIT OUT — separate follow-up (NOT P21.7 contribution-back; still unwired) · _deferred-feature_ · M · status=closed
  - sources: LD-F05 LD-H04 LD-D04 ADR-030 ADR-068

## P21.7

- **BL-039** — MapRoulette client + OSM changeset feed + §7 leverage metric page LANDED (P21.7/ADR-069; dry-run+fixtures tested); live push/pull gate-pending HG-08 · _deferred-feature_ · L · status=open · gate HG-08
  - sources: RISK-P16-14 RISK-P16-15 LD-F11 ADR-055 ADR-069 RISK-P18-14 LD-V09
- **BL-040** — Moderated usability study (>=5 ontology-naive contributors); protocol + opt-in aggregate-only instrumentation LANDED (P21.7/ADR-069); study gate-pending HG-10 · _operational-prereq_ · M · status=open · gate HG-10
...
- **BL-042** — Data Driven + coarse-international + France/Belgium live connectors · _deferred-feature_ · L · status=open · gate HG-03
  - sources: RISK-P18-04 RISK-P18-05 RISK-P18-06 RISK-P18-13 ADR-056 ADR-057 ADR-070
...
- **BL-055** — Live-fetch operationalization: Overpass robots/endpoint policy decision + document-connector content-drift hardening; wire real data/live_targets.toml rows + run real fetches into the hosted spine (P25.1..P25.6; findings from the P24.1 live deploy, ADR-082); implements the adopted ADR-083 API-vs-crawl carve-out — the umbrella backlog home for the whole P25 live-fetch series (see SOURCE_LIVE_OPS_MATRIX.md) · _external-dep_ · L · status=open · gate HG-03
  - sources: ADR-082 ADR-083 ADR-085 ADR-086 ADR-087 ADR-088 ADR-089
...
- **BL-038** — OSM/MapRoulette accounts + Organised-Editing activity registration (client+gate landed P21.7/ADR-069; registration itself gate-pending HG-08) · _external-dep_ · S · status=open · gate HG-08
...
- **BL-056** — P27 public-launch readiness: turn the fixture-demo web into a data-driven national public surface over the real ~1.06M-claim spine — ADR-090 (national surface from a spine-built export), ADR-091 (static core + React islands, DECISION-SPA=B), ADR-092 (spine-built export + resolution posture), ADR-093 (IA redesign), ADR-094 (rights resolution), ADR-097 (three public interactive islands realising ADR-091), ADR-098 (custom-domain cut-over to surveillancegraph.org via an external HTTPS LB); umbrella home for the P27 launch round, manifest rows 117-126 (P27.1..P27.10); ADR-109 (P31.2 HARDEN.2: one ingest_run per execution + append-only ingest_run_completion + the WORM run-row backfill — the engine half of D-P30.3-1; the public freshness page is owed to the P31.16 republish); ADR-117 (P31.14 SURFACE.1: the export emits the web/analytics/ presentation-analytics family — H3 density bins, degree centrality+focus, the derived decision point, provenance W-tiers, queue meta — schema/as_of/named-denominator envelopes, licence-separated web vs restricted web_mixed, fail-loud web seam; closes D-P27.5-1, the public half of D-P30.3-3 is owed to the P31.16 republish); ADR-118 (P31.15 SURFACE.2: per-compartment z0-z14 PMTiles via pinned tippecanoe + deterministic pure-Python fallback; the combined /map/points.json retired (R8-1 ending); repo-owned sig-web image — compiled Brotli + gzip + PMTiles ranges — and the first digest-pinned deploy path; public/live half owed to the P31.16 republish) · _deferred-feature_ · L · status=open · gate HG-01;HG-11;HG-02;HG-03
  - sources: ADR-090 ADR-091 ADR-092 ADR-093 ADR-094 ADR-095 ADR-096 ADR-097 ADR-098 ADR-106 ADR-109 ADR-117 ADR-118

## P28+

- **BL-057** — Round 6/7 depth & activation: activate the built-but-dormant depth machinery on the real ~1.06M-claim spine (Round 6) + turn on the human and accountability loops (Round 7) — ADR-099 (materialize entity resolution at scale over the spine + the measured LLM-adjudicator/human-holdout eval loop; supersedes ADR-092 compute-on-read for the resolved layer), ADR-100 (identity minimization — anonymous + OSM-mediated + issue-based tier-token curators; no homegrown accounts; OAuth-external-IdP deferred), ADR-101 (public surface reads the materialized graph, reconciling ADR-092); umbrella home for the Round 6/7 chain, manifest rows 127-135 (P28.1..P28.6 DEPTH + P29.1..P29.3 ACTIVATE); homes deferrals D-R6.1-EVAL + D-R7.1-AUTH; ADR-102 (P30.1 GO-LIVE.1: operator-approved temporary Cloud SQL scale-up for the OSM land + the settled audit snapshot status) homes the write-path throughput follow-ups D-P30.1-1 (batched sink writes) + D-P30.1-2 (incremental restarts); ADR-103 (P30.2 GO-LIVE.2: least-privilege sig_materialize role + in-GCP materializer execution + the hosted-scale deviations) homes D-P30.2-1 (coverage peer-class refinement) + D-P30.2-2 (entity-ref claims) + D-P30.2-3 (camera-registry predicate-registry coverage); ADR-104 (P30.2a GO-LIVE.2a: the 14 camera-registry predicates + the camera_registry/connector_run genres registered in the predicate registry, an absolute per-predicate coordinate tolerance, capture-time dating of undated claims — the §28 value-resolution half of D-P30.2-3) homes D-P30.2a-1 (assess the new genres for the non-camera predicates + register the non-camera hosted predicates) + D-P30.2a-2 (record re-sightings so capture-time dating tracks the latest assertion); ADR-105 (P30.2b GO-LIVE.2b: geospatial camera-site ENTITY resolution — a resolved site is a post-ER cluster of observation-level records of the same physical device, N of M; sized geo blocking, tiered same-device rules with test-pinned same-source / device-class / mirror-lineage constraints, a measured strict holdout gate, append-only camera_site_match + camera_site_run — the entity-dedup half of D-P30.2-3) homes D-P30.2b-1 (feed accepted review decisions into clustering) + D-P30.2b-2 (name-level device-kind / place conflicts) + D-P30.2b-3 (within-source duplicate layers); ADR-107 (P30.4 GO-LIVE.4: Cloud SQL steady-state tier db-custom-1-3840 after launch) homes D-P30.4-1 (sig-api reconnect after a DB restart) + D-P30.4-2 (bounded /v1/search) + D-P30.4-3 (entity-creation race, ENTITY-RACE-01) + D-P30.4-4 (armed project-id leak check vs committed build memory); ADR-108 (P31.1 HARDEN.1: read-API connection pool with purge-then-retry reconnect + the bounded /v1/search contract — min 3-alnum query, limit 50/200, keyset next_cursor, pg_trgm GIN index; code half of D-P30.4-1, closes D-P30.4-2) homes D-P31.1-1 (annotation watermark cost) + D-P31.1-2 (scheduled sig-probe image lacks the /health target) + D-P31.1-3 (/v1/entity 404 on an unregistered predicate); ADR-110 (P31.3 HARDEN.3: the entity_identity_key identity guard + batched chunk writes + the sink extension points; closes D-P30.4-3 and D-P30.1-1) homes D-P31.3-1 (hosted new-claim land rate, measured by P31.4); ADR-111 (P31.4 HARDEN.4: restart = resume — per-capture flush, append-only ingest_run_capture marks under a cadence-window logical run, a GCS capture store for hosted ingest jobs, every job rolled by pinned digest; closes D-P30.1-2, D-P31.3-1 and D-P31.1-2) homes D-P31.4-1 (verify the 2026-10-10 batch-05 OSM replay on the rolled image); ADR-112 (P31.5 DEEPEN.1: partner organisations as entity-ref claims — one identifier scheme (a crosswalk id, else the sig.org.name normalized name), the ambiguity and never-a-person rules, the link()-stage emitters, the partner schemes guarded with their backfill, the procurement + accountability predicate family registered; advances D-P30.2-2 and D-P30.2a-1) homes D-P31.5-1 (edges > 0 from emitted claims — the access-edge claims are P31.6) + D-P31.5-2 (read surfaces honour publication_review_required for partner organisations); ADR-113 (P31.6 DEEPEN.2: asserting replay of persisted captures — ingest_run_capture marks resolve OCFL bytes into connector post-capture with no network, asserting through a fresh is_replay PgClaimSink run with replay:-prefixed lineage marks and +0-by-dedupe re-runs; advances D-P30.2-2); ADR-114 (P31.7 DEEPEN.3: claim re-sightings — every live re-assertion appends a claim_evidence link to the execution's capture via the wired on_duplicates hook, uncapped and measured against the live disk; latest-capture dating capture_retrieved_at_latest so A→B→A resolves to A; SECURITY DEFINER close_superseded_resolutions keeps sig_materialize INSERT-only while superseding changed-digest decisions; closes D-P30.2a-2); ADR-115 (P31.9 DEEPEN.5: peer-class negative space — declared (entity_type, ingest_run.connector_name) classes with declared tracked-predicate sets in inference/data/peer_classes.toml, measured 1,035,351 would-be rows (25.8x below the unscoped 26.7M) vs the live-read 15 GB disk before writing; closes D-P30.2-1); ADR-116 (P31.11 DEEPEN.7: human review decisions into camera-site clustering — the run folds append-only review_decision history per pair (accept→human_accept same_as under the same hard constraints with decided_by=curator, reject→human_reject hard cannot_link, accept+reject disagreement→conflict routed back under er_match:camera_site_conflict:, constraint-violating accepts recorded refused never applied, verdicts precede auto outcomes, identity_duplicate stays with identity triage, zero decisions reproduce the pre-wiring run); plus evidenced duplicate-target lineage closing D-P30.2b-3 (row-identical or byte-identical captures + same ref → one effective record, the sole constraint-(a) exception, tier-0 duplicate_target_of edges; distinct same-source devices never merge); advances D-P30.2b-1 to Round-10 closure) · _deferred-feature_ · L · status=open · gate HG-11;HG-08;HG-10;HG-03
  - sources: ADR-099 ADR-100 ADR-101 ADR-102 ADR-103 ADR-104 ADR-105 ADR-107 ADR-108 ADR-110 ADR-111 ADR-112 ADR-113 ADR-114 ADR-115 ADR-116
...
- **BL-056** — P27 public-launch readiness: turn the fixture-demo web into a data-driven national public surface over the real ~1.06M-claim spine — ADR-090 (national surface from a spine-built export), ADR-091 (static core + React islands, DECISION-SPA=B), ADR-092 (spine-built export + resolution posture), ADR-093 (IA redesign), ADR-094 (rights resolution), ADR-097 (three public interactive islands realising ADR-091), ADR-098 (custom-domain cut-over to surveillancegraph.org via an external HTTPS LB); umbrella home for the P27 launch round, manifest rows 117-126 (P27.1..P27.10); ADR-109 (P31.2 HARDEN.2: one ingest_run per execution + append-only ingest_run_completion + the WORM run-row backfill — the engine half of D-P30.3-1; the public freshness page is owed to the P31.16 republish); ADR-117 (P31.14 SURFACE.1: the export emits the web/analytics/ presentation-analytics family — H3 density bins, degree centrality+focus, the derived decision point, provenance W-tiers, queue meta — schema/as_of/named-denominator envelopes, licence-separated web vs restricted web_mixed, fail-loud web seam; closes D-P27.5-1, the public half of D-P30.3-3 is owed to the P31.16 republish); ADR-118 (P31.15 SURFACE.2: per-compartment z0-z14 PMTiles via pinned tippecanoe + deterministic pure-Python fallback; the combined /map/points.json retired (R8-1 ending); repo-owned sig-web image — compiled Brotli + gzip + PMTiles ranges — and the first digest-pinned deploy path; public/live half owed to the P31.16 republish) · _deferred-feature_ · L · sources: ADR-090 ADR-091 ADR-092 ADR-093 ADR-094 ADR-095 ADR-096 ADR-097 ADR-098 ADR-106 ADR-109 ADR-117 ADR-118

## P28+ unscheduled (post-Round-4 manifest — the next planning round owns these)

Triaged by P24.5 (META.1 / GL-META-01, 2026-09-13): each row was checked against the tree — none is landed, none is owned by a remaining manifest ticket (rows 84–87 are JURIS.2/CCOPS.1/REC.1/GATE-ACCEPT). `P25+` = the planning round after this manifest, replacing the stale `P22+` bucket (the P22 pass closed unscheduled). Rows conditioned on real post-go-live state are additionally promoted to `docs/tickets/DEFERRALS.md` (D-META.1-*; the BL row stays the normalized-debt record).

- **BL-057** — Round 6/7 depth & activation: activate the built-but-dormant depth machinery on the real ~1.06M-claim spine (Round 6) + turn on the human and accountability loops (Round 7) — ADR-099 (materialize entity resolution at scale over the spine + the measured LLM-adjudicator/human-holdout eval loop; supersedes ADR-092 compute-on-read for the resolved layer), ADR-100 (identity minimization — anonymous + OSM-mediated + issue-based tier-token curators; no homegrown accounts; OAuth-external-IdP deferred), ADR-101 (public surface reads the materialized graph, reconciling ADR-092); umbrella home for the Round 6/7 chain, manifest rows 127-135 (P28.1..P28.6 DEPTH + P29.1..P29.3 ACTIVATE); homes deferrals D-R6.1-EVAL + D-R7.1-AUTH; ADR-102 (P30.1 GO-LIVE.1: operator-approved temporary Cloud SQL scale-up for the OSM land + the settled audit snapshot status) homes the write-path throughput follow-ups D-P30.1-1 (batched sink writes) + D-P30.1-2 (incremental restarts); ADR-103 (P30.2 GO-LIVE.2: least-privilege sig_materialize role + in-GCP materializer execution + the hosted-scale deviations) homes D-P30.2-1 (coverage peer-class refinement) + D-P30.2-2 (entity-ref claims) + D-P30.2-3 (camera-registry predicate-registry coverage); ADR-104 (P30.2a GO-LIVE.2a: the 14 camera-registry predicates + the camera_registry/connector_run genres registered in the predicate registry, an absolute per-predicate coordinate tolerance, capture-time dating of undated claims — the §28 value-resolution half of D-P30.2-3) homes D-P30.2a-1 (assess the new genres for the non-camera predicates + register the non-camera hosted predicates) + D-P30.2a-2 (record re-sightings so capture-time dating tracks the latest assertion); ADR-105 (P30.2b GO-LIVE.2b: geospatial camera-site ENTITY resolution — a resolved site is a post-ER cluster of observation-level records of the same physical device, N of M; sized geo blocking, tiered same-device rules with test-pinned same-source / device-class / mirror-lineage constraints, a measured strict holdout gate, append-only camera_site_match + camera_site_run — the entity-dedup half of D-P30.2-3) homes D-P30.2b-1 (feed accepted review decisions into clustering) + D-P30.2b-2 (name-level device-kind / place conflicts) + D-P30.2b-3 (within-source duplicate layers); ADR-107 (P30.4 GO-LIVE.4: Cloud SQL steady-state tier db-custom-1-3840 after launch) homes D-P30.4-1 (sig-api reconnect after a DB restart) + D-P30.4-2 (bounded /v1/search) + D-P30.4-3 (entity-creation race, ENTITY-RACE-01) + D-P30.4-4 (armed project-id leak check vs committed build memory); ADR-108 (P31.1 HARDEN.1: read-API connection pool with purge-then-retry reconnect + the bounded /v1/search contract — min 3-alnum query, limit 50/200, keyset next_cursor, pg_trgm GIN index; code half of D-P30.4-1, closes D-P30.4-2) homes D-P31.1-1 (annotation watermark cost) + D-P31.1-2 (scheduled sig-probe image lacks the /health target) + D-P31.1-3 (/v1/entity 404 on an unregistered predicate); ADR-110 (P31.3 HARDEN.3: the entity_identity_key identity guard + batched chunk writes + the sink extension points; closes D-P30.4-3 and D-P30.1-1) homes D-P31.3-1 (hosted new-claim land rate, measured by P31.4); ADR-111 (P31.4 HARDEN.4: restart = resume — per-capture flush, append-only ingest_run_capture marks under a cadence-window logical run, a GCS capture store for hosted ingest jobs, every job rolled by pinned digest; closes D-P30.1-2, D-P31.3-1 and D-P31.1-2) homes D-P31.4-1 (verify the 2026-10-10 batch-05 OSM replay on the rolled image); ADR-112 (P31.5 DEEPEN.1: partner organisations as entity-ref claims — one identifier scheme (a crosswalk id, else the sig.org.name normalized name), the ambiguity and never-a-person rules, the link()-stage emitters, the partner schemes guarded with their backfill, the procurement + accountability predicate family registered; advances D-P30.2-2 and D-P30.2a-1) homes D-P31.5-1 (edges > 0 from emitted claims — the access-edge claims are P31.6) + D-P31.5-2 (read surfaces honour publication_review_required for partner organisations); ADR-113 (P31.6 DEEPEN.2: asserting replay of persisted captures — ingest_run_capture marks resolve OCFL bytes into connector post-capture with no network, asserting through a fresh is_replay PgClaimSink run with replay:-prefixed lineage marks and +0-by-dedupe re-runs; advances D-P30.2-2); ADR-114 (P31.7 DEEPEN.3: claim re-sightings — every live re-assertion appends a claim_evidence link to the execution's capture via the wired on_duplicates hook, uncapped and measured against the live disk; latest-capture dating capture_retrieved_at_latest so A→B→A resolves to A; SECURITY DEFINER close_superseded_resolutions keeps sig_materialize INSERT-only while superseding changed-digest decisions; closes D-P30.2a-2); ADR-115 (P31.9 DEEPEN.5: peer-class negative space — declared (entity_type, ingest_run.connector_name) classes with declared tracked-predicate sets in inference/data/peer_classes.toml, measured 1,035,351 would-be rows (25.8x below the unscoped 26.7M) vs the live-read 15 GB disk before writing; closes D-P30.2-1); ADR-116 (P31.11 DEEPEN.7: human review decisions into camera-site clustering — the run folds append-only review_decision history per pair (accept→human_accept same_as under the same hard constraints with decided_by=curator, reject→human_reject hard cannot_link, accept+reject disagreement→conflict routed back under er_match:camera_site_conflict:, constraint-violating accepts recorded refused never applied, verdicts precede auto outcomes, identity_duplicate stays with identity triage, zero decisions reproduce the pre-wiring run); plus evidenced duplicate-target lineage closing D-P30.2b-3 (row-identical or byte-identical captures + same ref → one effective record, the sole constraint-(a) exception, tier-0 duplicate_target_of edges; distinct same-source devices never merge); advances D-P30.2b-1 to Round-10 closure) · _deferred-feature_ · L · sources: ADR-099 ADR-100 ADR-101 ADR-102 ADR-103 ADR-104 ADR-105 ADR-107 ADR-108 ADR-110 ADR-111 ADR-112 ADR-113 ADR-114 ADR-115 ADR-116
