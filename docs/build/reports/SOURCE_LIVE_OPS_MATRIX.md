...
| france/belgium (4) | `raa_prefectures` ✅ flipped (ODbL-1.0), live blocked on a resources→orders adapter; `decp_fr` ✅ flipped+LIVE (9,494 claims from the real DECP monthly file); `madada`, `declarationcamera_be` still HELD (no licence basis — HG-04 outreach owed) | API allow-list (`data.gouv.fr`) | none | green for the resolved two; LO 2.0 counsel flag retained | **P25.5** — per-source France gate (operator 2026-09-15) |
