...
| coarse-international (3) | `aspi_mapping_chinas_tech_giants`, `carnegie_ai_gsi`, `facial_recognition_world_map` | **LIVE document-capture** (P25.5): `carnegie_ai_gsi` + `facial_recognition_world_map` fetch their upstream pages as EvidenceArtifacts; `aspi` fetches but the host's WAF 403 is a recorded disappearance (never defeated) | some data.gov | FLIPPED on counsel approval (HG-02, 2026-09-15), derived-facts basis | **P25.4** |
| france/belgium (4) | `raa_prefectures` ✅ flipped (ODbL-1.0) + **LIVE document-capture** (the national RAA index CSV as an EvidenceArtifact; per-prefecture PDF→arrêté extraction is the owed adapter); `decp_fr` ✅ flipped+LIVE (9,494 claims from the real DECP monthly file); `madada` ✅ FLIPPED + **LIVE document-capture** (the platform's own successful-requests Atom feed); `declarationcamera_be` FLIPPED on counsel approval (HG-02) but stays NoLiveTargets — the register sits behind Belgian eID (HG-04 owed) | API allow-list (`data.gouv.fr`) | none | green for the resolved two; LO 2.0 counsel flag retained | **P25.5** — per-source France gate (operator 2026-09-15) |
| pathways / Stage-5 (3) | `pathways_rtcc_federation`, `pathways_fr_css_forensics`, `pathways_acoustic_drone_location` | **LIVE document-capture** (P25.5): one real EFF upstream page per family fetched as an EvidenceArtifact; field claims stay on the curated path | none | FLIPPED on derived-facts basis (ADR-085) | **P25.5** |
| CCOPS (3) | `ccops_seattle`, `ccops_nyc_post`, `ccops_sf` | **LIVE document-capture** (P25.5): `ccops_seattle` + `ccops_nyc_post` fetch their index pages as EvidenceArtifacts; `ccops_sf` refuses honestly — sf.gov's robots.txt is unretrievable to programmatic egress (SIG-INGEST-012) | none | FLIPPED on mandated-disclosure basis (ADR-085) | **P25.5** (+ BL-054 expansion) |
