...
| LD-V08 | P15.x (run, caveat) | Web verified with astro preview / chromium e2e over **fixture data**; never rendered from the live API or exports | `L=…p15-1…:137`; P15.1 ledger L43–47 (dossier/map/watch/corrections routes = later tickets); `L=…p18-1…:56` "Live web render deferred to P15/P18.2" | **CROSSED P21.4** — `web/src/lib/data.ts` (`SIG_DATA_SOURCE=export`) builds the site from the `sig-exports build --jurisdiction okc` output (`web/dossiers.json`); the OKC dossier renders the 299-vs-190 contradiction from the export bytes, asserted by `tests/e2e/test_composed_stack.py::test_s8_web_build_renders_from_export_bytes` (ADR-066). `fixtures` stays the CI default (back-compat). |
