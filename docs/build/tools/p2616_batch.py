...
# Flipped sources that also carry NEW wired targets — (state, scheme) for the
# generated rows (the source blocks predate this pass; the geo is reviewed).
FLIP_SOURCE_GEO: dict[str, tuple[str, str]] = {
    "camreg_md_opendata": ("MD", "us.state_abbr"),
    "camreg_nzta_nz": ("NZ", "iso.3166_1_alpha2"),
    "dot_511_ga": ("GA", "us.state_abbr"),
}
...
def _flip_source_block(text: str, source_id: str, licence: str, packet: str) -> str:
    ...
    # Preserve an existing terms_url (e.g. St. Albert's licence page); else
    # use the source's own homepage_url as the terms record.
    old_terms = ""
    tm = re.search(r'^terms_url = "(.*)"$', rights_block, re.M)
    if tm:
        old_terms = f'terms_url = "{tm.group(1)}"\n'
    else:
        hm = re.search(r'^homepage_url = "(.*)"$', main, re.M)
        if hm:
            old_terms = f'terms_url = "{hm.group(1)}"\n'
    rights = (
...
def _target_block(d: Disposition) -> str:
    ...
    elif d.source_id in FLIP_SOURCE_GEO:
        state, scheme = FLIP_SOURCE_GEO[d.source_id]
        agency = r.attribution or r.owner
    else:
        raise SystemExit(
            f"no geo spec for wired row {r.id} under source {d.source_id!r}"
        )
    if not state or not scheme:
        raise SystemExit(f"empty state/scheme for wired row {r.id}")
