...
# P15.3 — Infrastructure map + network explorer (§§39.3, 39.4, 19.5)

The two spatial/graph public surfaces, built on P15.1's epistemic visual language and no-JS/a11y
baseline (ADR-051). Implementation is `web/`; TypeScript is confined there (SIG-ENG-010). The
honest-rendering rules are pure logic in `web/src/lib/map.ts` and `web/src/lib/network.ts`; the
static-PMTiles serving contract is `web/src/lib/map-tiles.ts` served at `/map/style.json`. The
P15.1 reference map/graph are untouched (additive).

| Requirement | Where | Test |
|---|---|---|
| SIG-UI-016 (§39.3 layers; derived FOV/coverage separately toggled + visually distinct, SIG-GEO-006) | `web/src/lib/map.ts::OBSERVED_LAYERS`/`DERIVED_LAYERS`/`LAYER_CONTROLS`; `src/pages/map.astro` | `web/tests/unit/map.test.ts` "the §39.3 layer catalog" |
| SIG-UI-017 (coverage underlay bound to the point layer by a single control) | `web/src/lib/map.ts::POINT_COVERAGE_CONTROL`/`coverageBoundToPoints`/`assertCoverageBinding`; `src/pages/map.astro` (`data-bound`) | `web/tests/unit/map.test.ts` "coverage bound to the point layer by a single control"; `web/tests/e2e/map-network.spec.ts` "coverage underlay is bound … by a single control" |
| SIG-UI-018 (low-coverage MUST NOT read as low-density; desaturation/value-suppression) | `web/src/lib/map.ts::coverageEncoding`/`readsAsDensity`/`renderBin`; `src/pages/map.astro` (hatched, `data-reads-as-density`) | `web/tests/unit/map.test.ts` "low coverage MUST NOT read as low density"; `web/tests/e2e/map-network.spec.ts` + `map-network.nojs.spec.ts` |
| SIG-UI-019, SIG-GEO-011 (national-zoom density binning; points only where zoom supports precision) | `web/src/lib/map.ts::renderModeForZoom`/`pointVisibleAtZoom`/`MIN_POINT_ZOOM_BY_TIER`/`NATIONAL_ZOOM_MAX` | `web/tests/unit/map.test.ts` "national-zoom density binning + per-tier point honesty" |
| SIG-UI-020 (no-coordinate assets as jurisdiction indicators, never dropped) | `web/src/lib/map.ts::partitionByLocatability`/`isLocatable`; `src/pages/map.astro` (jurisdiction indicators) | `web/tests/unit/map.test.ts` "no-coordinate assets as jurisdiction indicators, never dropped"; `web/tests/e2e/map-network.spec.ts` (conservation) + `map-network.nojs.spec.ts` |
| SIG-UI-021 (sharing edges default to an ego network, not a hairball; matrix/arc alternatives) | `web/src/lib/map.ts::SHARING_EDGE_VIEWS`/`DEFAULT_SHARING_EDGE_VIEW`; `src/pages/map.astro` | `web/tests/unit/map.test.ts` "sharing edges default to an ego network" |
| SIG-UI-022 (network default = ego network with expansion) | `web/src/lib/network.ts::egoNetwork`/`DEFAULT_NETWORK_VIEW`; `src/pages/network.astro` (`data-default-view`) | `web/tests/unit/network.test.ts` "ego network with expansion is the default"; `web/tests/e2e/map-network.spec.ts` "defaults to an ego network" |
| SIG-UI-023, SIG-IDENT-030 (inline ER-quality disclosure on EVERY centrality/hub statistic) | `web/src/lib/network.ts::centralityStatistic`/`erDisclosureText`/`assertErDisclosures` (disclosure structurally required); `src/pages/network.astro` (`er-disclosure`) | `web/tests/unit/network.test.ts` "inline ER-quality disclosure on EVERY centrality statistic"; `web/tests/e2e/map-network.spec.ts` + `map-network.nojs.spec.ts` |
| SIG-UI-024 (three access edge types visually distinct + independently filterable, not merged by default, §12.2) | `web/src/lib/network.ts::ACCESS_EDGE_TYPES`/`ACCESS_EDGE_STYLES`/`filterAccessEdges`/`MERGE_ACCESS_EDGES_BY_DEFAULT`; `src/pages/network.astro` (legend + three groups) | `web/tests/unit/network.test.ts` "three access edge types, distinct + independently filterable"; `web/tests/e2e/map-network.spec.ts` |
| SIG-UI-025, SIG-RECON-050 (access-path closure: full hop list + per-hop evidence; speculative beyond threshold) | `web/src/lib/network.ts::assertPathValid`/`pathConfidence`/`pathIsSpeculative`/`pathIsHeadline`/`SPECULATIVE_HOP_THRESHOLD`; `src/pages/network.astro` | `web/tests/unit/network.test.ts` "access-path closure: full hops, per-hop evidence, speculative label"; `web/tests/e2e/map-network.spec.ts` |
| SIG-UI-038, SIG-GEO-012, SIG-GEO-013 (self-hosted static PMTiles v3; OSM attribution every context; no hard third-party CDN) | `web/src/lib/map-tiles.ts::PUBLIC_MAP_STYLE`/`assertServingContract`/`isSelfHostedTileUrl`/`OSM_ATTRIBUTION`; `src/pages/map/style.json.ts`; `src/pages/map.astro` (attribution line, incl. print) | `web/tests/unit/map-tiles.test.ts`; `web/tests/e2e/map-network.spec.ts` "served from static PMTiles v3 …"; `map-network.nojs.spec.ts` (attribution in static HTML) |
| Phase gate (SIG-ENG-004; §51.3): map has a tabular equivalent, network a list equivalent, usable no-JS; a11y + perf gates hold on the new pages | `src/pages/map.astro` (tables), `src/pages/network.astro` (lists); `tests/e2e/pages.ts` (SHELL_PAGES ⊇ /map/, /network/ → A11Y_PAGES) | `web/tests/e2e/map-network.nojs.spec.ts`; `web/tests/e2e/a11y.spec.ts` (axe WCAG 2.2 AA on /map/ + /network/); `lighthouserc.json` (0 script bytes, ≤150 KB on both pages) |

# P15.4 — Renewal watch + evidence recommender + evidence viewer (§§39.5, 39.5a, 39.6)

The three actionable-timing / evidence-inspection public surfaces, built on P15.1's
epistemic visual language and no-JS/a11y baseline and P15.2's `next_decision_date`
wire contract (ADR-052). Implementation is `web/`; TypeScript is confined there
(SIG-ENG-010). Additive — prior tickets' surfaces and wire names are untouched.

| Requirement | Where | Test |
|---|---|---|
| SIG-UI-026 (per contract: expiry, renewal window, notice deadline, approving body, next meeting, replacement procurement) | `web/src/lib/watch.ts::ContractWatchItem`/`resolveWatchItem`; `src/pages/watch.astro` (`watch-item`) | `web/tests/unit/watch.test.ts` "the watch keys on next_decision_date…"; `web/tests/e2e/watch-evidence.spec.ts` "every alert keys on next_decision_date…" |
| SIG-UI-027 (subscriptions by jurisdiction, iCal + RSS, keyed on `next_decision_date`) | `web/src/lib/watch.ts::toICal`/`toRss`/`alertItemsForJurisdiction`; `src/pages/watch/[jurisdiction].ics.ts`, `.../[jurisdiction].xml.ts` | `web/tests/unit/watch.test.ts` "iCal subscription is valid…"/"RSS subscription is valid…"; `web/tests/e2e/watch-evidence.spec.ts` "subscriptions are offered by jurisdiction…" |
| SIG-UI-014b (watch keys alerts on `next_decision_date`, not expiry — reused, not forked) | `web/src/lib/watch.ts::resolveWatchItem` (delegates to `dossier.resolveTermination`) | `web/tests/unit/watch.test.ts` "derives the notice deadline = expiry − notice window"; `web/tests/e2e/watch-evidence.spec.ts` + `.nojs.spec.ts` |
| SIG-UI-027a (rank most-useful evidence by the six §39.5a inputs only; J-3 executable) | `web/src/lib/recommender.ts::recommendEvidence`/`scoreBreakdown`/`isAdmissible`; `src/pages/watch.astro` (`recommended-artifact`) | `web/tests/unit/recommender.test.ts` "ranks by the §39.5a admissible inputs only"; `web/tests/e2e/watch-evidence.spec.ts` "ranks by directness/recency/dispute only…" |
| SIG-UI-027b (neutrality guard: NOT persuasiveness/sentiment/vote-effect) | `web/src/lib/recommender.ts::FORBIDDEN_RANKING_INPUTS`/`assertNeutralInputs`; the closed `EvidenceArtifact` type + pure `scoreArtifact` | `web/tests/unit/recommender.test.ts` "the neutrality guarantee (SIG-UI-027b)"; `web/tests/e2e/watch-evidence.spec.ts` "states the neutrality guarantee; the score uses no forbidden axis" |
| SIG-UI-027c (exportable citation list with permalinks + as-of dates) | `web/src/lib/recommender.ts::citationList`/`citationListText`; `src/pages/watch/citations.txt.ts`; `src/pages/watch.astro` (`citation-list`) | `web/tests/unit/recommender.test.ts` "exportable citation list with permalinks + as-of dates"; `web/tests/e2e/watch-evidence.spec.ts` "output is exportable as a citation list…" |
| SIG-UI-028 (viewer: document w/ span highlighted at locator; claim; extraction method+version; review status; conflicting claims; capture date+digest; acquisition method; full history) | `web/src/lib/evidence-viewer.ts::highlightSpan`/`assertClaimView`/`ClaimView`; `src/pages/evidence/[id].astro` | `web/tests/unit/evidence-viewer.test.ts` "renders the document with the supporting span highlighted"; `web/tests/e2e/watch-evidence.spec.ts` "renders the document with the supporting span highlighted at its locator" |
| SIG-UI-029, SIG-RECON-045 (diff two captures of the same artifact field-by-field; §29.7) | `web/src/lib/evidence-viewer.ts::diffCaptures`/`changedFields`; `src/pages/evidence/[id].astro` (`capture-diff`) | `web/tests/unit/evidence-viewer.test.ts` "diffs two captures of the same artifact field by field"; `web/tests/e2e/watch-evidence.spec.ts` + `.nojs.spec.ts` |
| SIG-UI-030, SIG-EVID-009/010 (sealed captures metadata-only + explanation; §17.5) | `web/src/lib/evidence-viewer.ts::sealedMetadata`/`assertCaptureTier`/`isSealed`; `src/pages/evidence/[id].astro` (`sealed-notice`) | `web/tests/unit/evidence-viewer.test.ts` "sealed captures are metadata-only with an explanation"; `web/tests/e2e/watch-evidence.spec.ts` "renders a sealed capture metadata-only with an explanation" |
| SIG-UI-008 (contested values marked at EVERY appearance: watch, feed text, recommender, export, conflicting-claims view) | `web/src/lib/watch.ts` (contested suffix in iCal/RSS); `recommender.ts` (`contested` on ranked + citation); `evidence-viewer.ts::isClaimContested`; `ContestedMarker.astro` in `watch.astro`/`evidence/[id].astro` | `web/tests/unit/watch.test.ts`/`recommender.test.ts`/`evidence-viewer.test.ts`; `web/tests/e2e/watch-evidence.spec.ts` "contested values are marked at every appearance" |
| Phase gate (SIG-ENG-004; §51.3): the three surfaces exist + function no-JS; a11y + perf gates hold on the new pages | `src/pages/watch.astro`, `src/pages/evidence/[id].astro`, `src/pages/evidence/index.astro`; `tests/e2e/pages.ts` (SHELL_PAGES ⊇ /watch/, /evidence/, /evidence/*/ → A11Y_PAGES) | `web/tests/e2e/watch-evidence.nojs.spec.ts`; `web/tests/e2e/a11y.spec.ts` (axe WCAG 2.2 AA on the new pages); `lighthouserc.json` (0 script bytes, ≤150 KB, auto-globbed over `dist`) |

# P15.5 — Research queue, corrections log, methodology + metrics, editorial standards (§§39.7, 39.8, 41)

The last Phase-15 public surfaces, built on P15.1's epistemic visual language and
no-JS/a11y baseline (ADR-053). Implementation is `web/`; TypeScript is confined there
(SIG-ENG-010). Additive — prior tickets' surfaces and wire names are untouched.

| Requirement | Where | Test |
|---|---|---|
| SIG-UI-031 (research queue: cards state closing condition, evidence sought, assignee class, effort estimate; geographic filtering; claiming with expiry; full disposition vocab incl. "searched, found nothing") | `web/src/lib/research-queue.ts::ResearchTaskCard`/`dispositionsFor`/`tasksForJurisdiction`/`claimStatusFor`/`orderQueue`/`SEARCHED_FOUND_NOTHING`; `src/pages/research-queue.astro` (`task-card`) | `web/tests/unit/research-queue.test.ts`; `web/tests/e2e/corrections-methodology.spec.ts` "research queue (§39.7…)"; `corrections-methodology.nojs.spec.ts` |
| SIG-TASK-010/011/012 (claim grants priority NOT exclusivity; expires without renewal; anyone may work; no volume leaderboard) | `web/src/lib/research-queue.ts::claimIsActive`/`claimStatusFor` (`anyone_may_work: true`)/`orderQueue`; `src/pages/research-queue.astro` (`anyone-may-work`, `no-leaderboard-note`) | `web/tests/unit/research-queue.test.ts` "claiming with expiry…"; `corrections-methodology.spec.ts` "claiming grants priority, never exclusivity" |
| SIG-TASK-008/009 (disposition vocabulary richer than done; resolved_no_evidence_exists on search work) | `web/src/lib/research-queue.ts::DISPOSITIONS`/`DISPOSITIONS_BY_ASSIGNEE`/`canRecordNoEvidence` | `web/tests/unit/research-queue.test.ts` "search-work tasks carry…"; `corrections-methodology.spec.ts` "exposes the full disposition vocabulary…" |
| SIG-UI-032 (public corrections log — the eighth surface — with what changed, when, why, who reported it) | `web/src/lib/corrections.ts::CorrectionEntry`/`orderedCorrections`; `src/pages/corrections.astro` (`correction`, `what-changed`/`when`/`why`/`reported-by`) | `web/tests/unit/corrections.test.ts` "corrections log"; `corrections-methodology.spec.ts` "public corrections log"; `.nojs.spec.ts` |
| SIG-GOV-005 (a correction is a new assertion; the prior value stays citable at its belief-time) | `web/src/lib/corrections.ts::priorValuePermalink`; `src/pages/corrections.astro` (`prior-permalink`) | `web/tests/unit/corrections.test.ts` "the prior value stays citable…"; `corrections-methodology.spec.ts` "preserves history" |
| SIG-GOV-004/007/008/011 (permitted outcomes incl. refusal + suppression≠deletion; transparency report by category/outcome incl. refusals) | `web/src/lib/corrections.ts::CORRECTION_OUTCOMES`/`CORRECTION_OUTCOME_META`/`transparencyReport`; `src/pages/corrections.astro`, `dispute.astro` (`refusal-outcome`) | `web/tests/unit/corrections.test.ts` "transparency reporting includes refusals"; `corrections-methodology.spec.ts` |
| SIG-UI-033, SIG-GOV-001/002/003 (dispute/correction path one click from any claim on every page; five §45.1 categories; privacy/safety prioritized; no identity except legal demand) | `web/src/lib/corrections.ts::disputeHref`/`SUBMISSION_CATEGORIES`/`categoriesByPriority`; `components/DisputeLink.astro` in `BaseLayout.astro`; `src/pages/dispute.astro` | `web/tests/unit/corrections.test.ts` "submission categories"/"one-click submission path"; `corrections-methodology.spec.ts` "dispute/correction submission path"; `.nojs.spec.ts` |
| SIG-UI-034, SIG-METRIC-006/007 (methodology + data-freshness + coverage-metrics pages, public + linked from every dossier; per-source freshness against predicate volatility) | `web/src/lib/metrics.ts::FreshnessRow`/`assertFreshnessRow`; `src/pages/methodology.astro`, `data-freshness.astro`, `coverage-metrics.astro`; links in `components/HowWeKnowThis.astro` (on every page ⊇ every dossier) | `web/tests/unit/metrics.test.ts` "data freshness"; `corrections-methodology.spec.ts` "all three pages … linked from every dossier" |
| SIG-METRIC-008/009/010 (never a total; named denominators; no capture–recapture; no completeness % implying a known denominator) | `web/src/lib/metrics.ts::CoverageMetric`/`assertCoverageMetric`/`assertCoverageMetrics` (`is_population_total: false`, forbidden-denominator regex); `src/pages/coverage-metrics.astro` (`no-total-note`), `methodology.astro` (`no-capture-recapture`) | `web/tests/unit/metrics.test.ts` "coverage metrics — never a total"; `corrections-methodology.spec.ts` "coverage metrics name denominators and never claim a total"/"methodology states the capture–recapture prohibition" |
| SIG-UI-044 ("How we know this" module on EVERY page with all six components: artifact counts, tier distribution, source-independence count, date range, rules applied, human-review status) | `web/src/lib/provenance.ts::ProvenanceSummary`/`PROVENANCE_COMPONENTS`/`assertProvenanceComplete`; `components/HowWeKnowThis.astro` in `BaseLayout.astro` | `web/tests/unit/provenance.test.ts`; `corrections-methodology.spec.ts` "'How we know this' module on every page" (iterates SHELL_PAGES); `.nojs.spec.ts` |
| SIG-UI-043/046 (six register rules enforced; a public style guide; rules applied to generated rationale as well as hand-written copy) | `web/src/lib/editorial.ts::REGISTER_RULES`/`checkRegisterConformance`/`assertRegisterConformant`/`FORBIDDEN_REGISTER_TERMS`; `src/pages/style-guide.astro` (build-gates `GENERATED_RATIONALE_TEMPLATES`) | `web/tests/unit/editorial.test.ts` "the six register rules"/"register conformance applies to generated text too"; `corrections-methodology.spec.ts` "the style guide codifies the six register rules" |
| SIG-UI-045 (three example editorial cases rendered as specified) | `web/src/lib/editorial.ts::EDITORIAL_CASES`; `src/pages/editorial-standards.astro` (`editorial-case`) | `web/tests/unit/editorial.test.ts` "the three example editorial cases"; `corrections-methodology.spec.ts` "the three example cases render as specified"; `.nojs.spec.ts` |
...
P21.6 builds the authenticated curation surface the Phase-5 ticket deferred (LD-F05/LD-H04): reviewers
work the ER review queue and contradiction/task dispositions in a browser and L0 contributors submit
through the web, every action landing as an append-only row with a human actor id (ADR-068). Part VIII
§0.7 is binding — the curation service is authenticated and never on the public API process. P21.2
shipped the shrunk case, so no new persistence was built: ER decisions use the JSONL curation log /
`PgReviewQueue`.`review_decision`; contradiction/task dispositions are compute-on-read.

| Requirement | Where | Test |
|---|---|---|
| Curation API disabled by default — routes structurally absent without `SIG_CURATION_ENABLED=1` (RISK-P21-10) | `api/src/api/curation.py` (`create_curation_app`, `curation_enabled`); `api/src/api/cli.py` (`serve-curation` exits 3 when disabled) | `tests/api/test_curation.py::test_routes_absent_when_disabled`, `::test_env_gating_default_is_disabled`, `::test_public_read_api_never_carries_curation_routes`; `tests/ops/test_curation_service.py::test_serve_curation_refuses_when_disabled` |
| Authenticated + tier-gated: no token → 401; insufficient tier → 403 (§36.1, §34.1) | `api/src/api/curation.py` (`authenticated_contributor`, `_require_scope`, `CURATION_KEYS` bound to `tasks.contributor` tiers) | `tests/api/test_curation.py::test_unauthenticated_is_401`, `::test_tier_insufficient_is_403` |
| Append-only writes: a decide writes exactly one row; repeat appends another; never mutates (P1–P3) | `api/src/api/curation.py` (`CurationLog.append`, review-queue `/decide`) | `tests/api/test_curation.py::test_decide_writes_exactly_one_row_and_repeats_append`, `::test_decided_item_drops_out_of_pending_compute_on_read` |
| No decision writable without a human actor id — enumerated across every write path (§3.1, SIG-LLM-002) | `api/src/api/curation.py` (`CurationLog.append` refuses empty actor; every route depends on `authenticated_contributor`) | `tests/api/test_curation.py::test_curation_log_refuses_empty_actor`, `::test_every_write_route_requires_auth_so_carries_an_actor`, `::test_all_router_post_paths_are_covered_by_the_enumeration` |
| Machine suggestions labelled + never auto-apply (amends ADR-030; SIG-LLM-001..007) | `api/src/api/curation.py` (`_item_to_row` `auto_applied:false`); `web/src/pages/curate/[id].astro`, `index.astro` (labelled suggestion, no pre-selected radio) | `tests/api/test_curation.py::test_model_suggestion_is_labelled_and_not_applied`, `::test_deciding_a_model_item_logs_model_provenance`; `web/tests/e2e/curate.spec.ts` (suggestion labelled; no radio checked) |
| Anti-poisoning server-side; device observations route to OSM; L0 refuses without evidence (SIG-CONTRIB-002/004/010) | `api/src/api/curation.py` (`/submission` uses `tasks.submission`, `tasks.poisoning`) | `tests/api/test_curation.py::test_submission_refuses_without_evidence`, `::test_submission_routes_device_observation_to_osm`, `::test_submission_lands_l0_append_only` |
| Contradiction dispositioned with both claims/evidence/dates + a reason; revert = new assertion (§3.1, §30, SIG-CONTRIB-009) | `api/src/api/curation.py` (`/contradiction/{id}/disposition`, `/revert`); `web/src/pages/curate/contradictions.astro`, `revert.astro` | `tests/api/test_curation.py::test_contradiction_disposition_requires_reason`, `::test_task_no_evidence_disposition_requires_sources`, `::test_revert_requires_reason_and_is_a_new_assertion`; `web/tests/e2e/curate.spec.ts` |
| `/curate/**` forms submit without JS; WCAG 2.2 AA on every page; public pages keep script size 0 (SIG-UI-036..041) | `web/src/pages/curate/**`; `web/src/layouts/CurateLayout.astro`; `web/src/lib/curation.ts`; `web/lighthouserc.json` (`/curate/**` block) | `web/tests/e2e/curate.nojs.spec.ts` (native POST forms, no `<script>`); `web/tests/e2e/curate.spec.ts` + `a11y.spec.ts` (axe AA on all `/curate/` pages); `web/tests/unit/curation.test.ts` |
| Separate `api-curation` process bound to a non-public interface; `sig-ops status` reports it separately | `ops/docker-compose.yml` (`api-curation`, 127.0.0.1); `ops/src/ops/cli.py` (`_curation_host_port`, up/status/down) | `tests/ops/test_curation_service.py::test_status_reports_curation_on_its_own_line`, `::test_compose_defines_api_curation_bound_to_loopback`, `::test_curation_url_is_loopback_by_default` |
| Phase gate (§51.3): `make check` green; ADR-068; register + traceability; BACKLOG rows closed | `make check`; `docs/adr/ADR-068-*.md` (+ Appendix F, README); `docs/risk_register.md` `## Phase 21 — Operationalization (P21.6)` (RISK-P21-10/11); `docs/build/CURATION_UI.md`; `docs/build/BACKLOG.csv` | `make check`; `python docs/build/tools/check_spec_src.py`; BL-009 `status=closed` |

# P21.9 — Stage-5 connectors for the Phase-17 pathways + the deferred parser layers

P21.9 does the ingestion half of Phase 17 (LD-H12, retiring RISK-P17-03): one `pathways` connector
with three per-family extractors (ADR-071) reads public procurement/policy/federation/deployment
documents and emits the P17 pathway claims as dated, evidenced claim-spine rows, through the two
ADR-033-deferred parser layers now realised in `parsing/` (LD-F17). The load-bearing invariant is the
§46 epistemic rule (RISK-P21-16): `procured` NEVER implies `deployed` — a deployment claim requires a
deployment-genre document, enforced by a genre-level and a predicate-level guard with the genre
re-derived from the document text. All runs are fixture replay (HG-03/HG-04 per source pending: SKIP —
no source flipped); a `run --mode live` refuses (exit 3).

| Requirement | Where | Test |
|---|---|---|
| ADR-071 coupling decision: one `pathways` connector + three extractors (shared parser layers/guard) | `connectors/src/connectors/pathways.py` (`PathwaysConnector`, `extract_documents` dispatcher); `data/pathways_vocab.toml` | `tests/connectors/test_pathways.py::test_connector_is_registered`, `test_pathway_coverage.py::test_vocab_roster_matches_the_conformance_suite` |
| Parser layer 4 (`pdf_table`) — procurement table → cell-located claims (SIG-PARSE-001/003, LD-F17) | `parsing/src/parsing/tables.py` (`parse_table`, `table_claims`, `TABLE_LAYER`) | `tests/parsing/test_tables.py` |
| Parser layer 3 (`pdf_text`) — policy document → byte-range clause locators (SIG-PARSE-003, LD-F17) | `parsing/src/parsing/clauses.py` (`locate_clauses`, `clause_claim`, `CLAUSE_LAYER`) | `tests/parsing/test_clauses.py` |
| Document-genre classification; `sig-parsing classify` recognises the new genres (LD-F17) | `parsing/src/parsing/genre.py` (`classify_genre`, `DocumentGenre`, `DEPLOYMENT_GENRES`); `parsing/src/parsing/cli.py` (`_run_classify` prints genre) | `tests/parsing/test_genre.py`; `tests/parsing/test_cli_parsing.py` |
| P17 claims typed + evidenced (vendor/product, procurement, policy, deployment) | `connectors.pathways.pathway_claim` (claim_type + evidence locator + observed_at) | `tests/connectors/test_pathways.py::test_family_emits_typed_evidenced_claims`, `::test_parser_layers_are_exercised` |
| EPISTEMIC RULE (§46, RISK-P21-16): `procured` ≠ `deployed`; deployment requires a deployment-genre document | `connectors.pathways.assert_claim_type_supported_by_genre` (`DeploymentInferenceError`), `assert_deployment_predicate_has_deployment_type`, `_document_context` (genre re-derived) | `tests/connectors/test_pathways.py::test_deployment_claim_from_procurement_genre_is_refused`, `::test_deployment_predicate_on_a_non_deployment_claim_is_refused`, `::test_mislabelled_genre_is_rejected_genre_is_re_derived_from_text`, `::test_no_deployment_claim_originates_from_a_procurement_genre_document` |
| Coverage: every P17 conformance pathway has ≥1 connector-produced claim → `STAGE5_CONNECTORS.md` (retires RISK-P17-03) | `tests/connectors/test_pathway_coverage.py`; `docs/build/STAGE5_CONNECTORS.md` | `tests/connectors/test_pathway_coverage.py::test_every_p17_pathway_has_at_least_one_connector_produced_claim`, `::test_stage5_connectors_report_is_present_and_consistent` |
| Registry rows + packets per new source; rights UNDETERMINED / `ingestion_permitted=false`; live REFUSES exit 3 | `connectors/src/connectors/data/sources.toml` (`pathways_*`); `docs/build/rights/pathways_*.md`; `tests/connectors/fixtures/pathways/SOURCES.md` | `tests/connectors/test_pathways.py::test_all_three_family_sources_keep_link_posture_and_are_gated`, `::test_fixtures_have_a_sources_md` |
...
## Phase 24 — Productionize (P24.3 — observability & alerting, OBS.1 / GL-OBS-01)

Stamped id: **GL-OBS-01**. ADR-077 (record-first/notify-second notifier seam; bounded JSONL
metrics/logs; budgets as a pure function; structural secret scrubbing). Builds on the INFRA.1
`ops.egress` alarm (consumed, not re-implemented) and the ADR-067 degraded/keepalive seams
(re-confirmed at build time: `ops/src/ops/egress.py` `build_report`/`exit_code_for`,
`ops/src/ops/degraded.py` `build_static_site`, `.github/workflows/keepalive.yml`).

| Requirement / obligation | Where implemented | Proof |
|---|---|---|
| GL-OBS-01 — an egress-threshold breach fires a RECORDED alert | `sig-ops egress-report --alert` (`ops/src/ops/cli.py` `_cmd_egress_report`) → `ops.alerts.fire` → append-only `AlertLedger`; the breach decision stays in `ops.egress.build_report` | `tests/ops/test_alerts.py::test_egress_breach_fires_a_recorded_alert` (exit 5 + ledger row kind=egress-budget severity=alarm), `test_egress_warn_band_records_a_warn_alert`, `test_egress_below_threshold_records_no_alert`, `test_egress_gate_pending_fires_no_false_alarm` |
| GL-OBS-01 — a keepalive failure fires a RECORDED alert | `sig-ops keepalive-check` → `observe.verify_keepalive` (workflow intact + real degraded rebuild) → `critical` alert; `keepalive.yml` `if: failure()` step runs `sig-ops alert` | `test_keepalive_failure_fires_a_recorded_alert` (exit 6 + ledger row), `test_verify_keepalive_reports_a_failed_rebuild`, `test_verify_keepalive_reports_a_missing_or_gutted_workflow`, `test_committed_keepalive_workflow_is_intact` |
| GL-OBS-01 — a dashboard/readout exists; no secrets appear in logs | `sig-ops dashboard` → `observe.render_dashboard` (markdown readout); committed artifact `docs/build/reports/observability-dashboard.md`; `alerts.scrub_secrets` masks env-secret values in every emitted surface | `test_render_dashboard_covers_health_budgets_egress_keepalive_alerts`, `test_render_dashboard_is_honest_when_nothing_is_recorded`, `test_cli_dashboard_writes_the_readout_file`; `test_no_env_secret_reaches_the_ledger_or_log`, `test_scrub_secrets_masks_env_secret_values`, `test_webhook_posts_scrubbed_json_and_never_logs_the_url` |
| GL-OBS-01 — metrics/logs for the live stack within the zero-cost posture | `sig-ops probe` → `observe.probe_stack` (pg/api/curation/static, ok/latency/ts) → bounded `ProbeLog`; `http_ok`/`pg_ready` are the single implementation `cli.status`/`up` reuse | `test_probe_stack_covers_every_stack_service`, `test_probe_log_appends_and_reads_back`, `test_cli_probe_records_metrics_and_alerts_on_down_services` |
| GL-OBS-01 — uptime + error budgets | `observe.compute_uptime` (rolling `window_days`, `uptime_target_pct` from `ops/config.toml [observability]`); `no-data` when unmeasured (§3.1) | `test_compute_uptime_within_budget` (99/100 → within-budget, 100% burn), `test_compute_uptime_exhausts_the_error_budget`, `test_compute_uptime_ignores_rows_outside_the_window_and_reports_no_data` |
| GL-OBS-01 — bounded log retention within the zero-cost posture | `observe.prune_jsonl` (age + byte caps from `[observability]`, oldest first) applied to the probe log + alert ledger (`--prune`, `dashboard`, `probe`) | `test_retention_prunes_rows_older_than_the_window`, `test_retention_enforces_the_byte_cap_oldest_first`, `test_prune_on_a_missing_log_is_a_noop` |
| Zero-cost + env-only secrets (SIG-STORE-003, HG-09) | notifier = ledger + log + optional `SIG_ALERT_WEBHOOK_URL`/`_TOKEN` webhook (no paid service, no literals); `ops/config.toml [observability]` documents env names only | `test_unconfigured_webhook_delivers_nothing_but_the_record_stands`, `test_no_env_secret_reaches_the_ledger_or_log`; `tests/connectors/test_secrets.py` green |
| RISK-P24-02 — alert path could leak env secrets, or a breach could fire nothing if recording depended on delivery | `docs/risk_register.md` Phase 24 row; `fire` records before notifying; `scrub_secrets` everywhere | `test_alert_ledger_is_append_only`, `test_recorded_alert...`/`test_unconfigured_webhook...`, the scrub tests above |

## Phase 24 — Productionize (P24.4 — CI composed hardening, CI.1 / GL-CI-01)

Stamped id: **GL-CI-01**. ADR-078 (composed job; offline vendored scanners; PR-vs-nightly
split; nightly report-is-the-gate; the starlette security fix the audit surfaced). Closes
the LEDGER open findings CI-RED-01 (composed e2e runs for real in CI with the web build
present) and DOCKER-DOWN (full DB + composed suite re-run green with
`SIG_REQUIRE_DB_TESTS=1`; CI now enforces the same).

| Requirement / obligation | Where implemented | Proof |
|---|---|---|
| GL-CI-01 — a CI job runs the composed `tests/e2e` for real (Node + Docker present → S8/LD-V08 web build runs) | `.github/workflows/ci.yml` `composed` job: `actions/setup-node@v4` + `npm --prefix web ci` + `SIG_REQUIRE_DB_TESTS=1 uv run pytest tests/e2e` on `ubuntu-latest` (Docker daemon present) | `tests/unit/test_ci_workflows.py::test_ci_composed_job_*` (job exists, runs tests/e2e, installs node_modules, fail-loud env); local identical run: `SIG_REQUIRE_DB_TESTS=1 uv run pytest tests/e2e` → 16 passed/0 failed incl. all three S8 web-build tests (fixtures + export-mode dossier + PMTiles) |
| GL-CI-01 — a nightly composed run that reports | `.github/workflows/nightly.yml` — cron `0 3 * * *` + `workflow_dispatch`; composed e2e + all three scans with `continue-on-error`; `scripts/ci/nightly_report.py` writes `nightly-report.md` and exits non-zero on any failed stage; `actions/upload-artifact` `if: always()` | `test_nightly_is_scheduled_and_manually_dispatchable`, `test_nightly_runs_composed_e2e_and_all_three_scans`, `test_nightly_e2e_fails_loudly_without_docker`, `test_nightly_reports_an_artifact_and_gates_on_failures`; `test_nightly_report_*` (green/RED verdicts, exit codes, file written) |
| GL-CI-01 — dependency scanning | `scripts/ci/dep_audit.sh` (`uv export --frozen --all-packages` → `uvx pip-audit` → OSV); `make audit-deps`; nightly stage | Real run: reported 12 known vulns in starlette 0.48.0 → fixed in this ticket (`fastapi>=0.141,<0.142` + `starlette>=1.3.1` floor in `api/pyproject.toml`; resolved 1.6.0) → re-audit "No known vulnerabilities found" |
| GL-CI-01 — licence scanning | `scripts/ci/license_scan.py` (stdlib `importlib.metadata`; DENY categories mirror `web/scripts/check-licenses.mjs`; unresolvable = review-required; strong-copyleft must equal frozen `EXPECTED_STRONG_COPYLEFT`) in the `security` job + nightly + `make scan-licenses` | `test_license_scan_*` — seeded BUSL/unresolvable/new-GPL records → exit 1; clean records → 0; real env → 0 (119 dists, flagged set = expected) |
| GL-CI-01 — secret scanning | `scripts/ci/secret_scan.py` (tracked-file scan, superset of the build-memory shape set + `SIG_*` credential literals; never echoes the match) in the `security` job + nightly + `make scan-secrets` | `test_secret_scan_*` — seeded AWS key / PEM / `SIG_*` literal → exit 1, match never echoed; real tree → 0; live seeded-file demo (exit 1, removed) |
