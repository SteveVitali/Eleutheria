...
| ADR-114 | Claim re-sightings on the spine, and latest-capture dating (P31.7 / DEEPEN.3, Round 9 — the design's ADR-R9-RESIGHT; closes D-P30.2a-2, revises ADR-104 §4's interim earliest-capture basis): the production `on_duplicates` hook `record_resightings` (wired by `make_claim_sink('pg')`) appends one `claim_evidence` `establishes` link per re-sighted claim to the execution's synthetic per-(source, genre, run) capture — every re-sighting recorded, uncapped (ratified Q5 2026-09-24), idempotent on the `(claim_id, capture_id, role)` PK, inside the chunk transaction, never on `is_replay` runs (a replay re-reads stored bytes — not a source sighting — so `DuplicateBatch.replay` skips it); undated claims are dated from the LATEST sighting (`max(retrieved_at)`, basis `capture_retrieved_at_latest` in `rules_fired`) so A → B → A resolves to A; sqitch `resolution_supersede` adds the SECURITY DEFINER `close_superseded_resolutions` (EXECUTE to `sig_materialize` only) that closes a live `auto` decision's `sys_period` when the batch's input_digest differs (§16.4 supersession — history kept, `sig_materialize` stays INSERT-only) and reports live non-auto pairs `blocked` (counted `skipped_pinned`, never auto-overridden); storage growth measured live against the 15 GB `sig-pg` disk (+ the operator's 10 GB comparison) with a 12-month projection and a 50%-of-disk revisit threshold in the ADR; rollout by digest via `roll-jobs --exclude sig-sched-camreg-batch-05` (batch-05 holds `feff986c…` for D-P31.4-1), bounded link-volume measurement first, resolution re-run +0 verified. BL-057 | P31.7 |
| ADR-115 | Peer-class negative space: declared `(entity_type, connector)` classes with declared tracked predicates (P31.9 / DEEPEN.5, Round 9 — the design's ADR-R9-PEERCLASS; closes D-P30.2-1): the P28.4 entity-type-as-class rule unioned every spine predicate per class — meaningless on the all-`deployment` hosted spine (measured 26,696,184 would-be `not_researched` rows, ~12× the spine, the reason ADR-103 ran coverage `--no-negative-space`). The peer class is now `(entity.entity_type, ingest_run.connector_name)` — membership via the claim→run→connector lineage, no schema change — with each class's `tracked` predicate set **declared** in `inference/data/peer_classes.toml` (versioned, validated at load; seeded from the measured per-connector emit surface minus per-claim bookkeeping predicates like `matched_keyword`/`unmapped_surveillance_tag`, whose absence is an extraction outcome, not a gap). Undeclared predicates are never negative space; undeclared connectors give a subject no coverage surface (never a guess); `not_researched`/`auto` vocabulary and `input_digest` +0 idempotency unchanged; `--no-negative-space` stays an opt-out flag but `ops/gcp/materialize.sh` no longer passes it. Measured read-only before write: **1,035,351** would-be rows (25.8× smaller; dot_511 907,291, procurement 118,076, osm 6,548, 9 smaller classes) ≈ 0.27 GB against the live-read 15 GB disk; named-denominator metrics untouched (SIG-METRIC-008). BL-057 | P31.9 |
| ADR-116 | Human review decisions into camera-site clustering, and evidenced duplicate-target lineage (P31.11 / DEEPEN.7, Round 9 — the design's ADR-R9-HUMANER; closes D-P30.2b-3, advances D-P30.2b-1): each camera-site run folds the append-only `review_decision` history for `er_match:camera_site*` items to one verdict per pair (latest decided item governs; accept/reject disagreement is a `conflict` routed back under `er_match:camera_site_conflict:`); `accept` applies as a `human_accept` same_as edge under the same hard constraints (`decided_by` the curator), `reject` records a hard `cannot_link`, a constraint-violating accept is recorded `refused` never applied, human verdicts precede every automatic outcome and stay outside the measured eval loop; zero decisions reproduce the pre-wiring run byte-identical (test-pinned); P31.3 `identity_duplicate` decisions stay with identity triage; the sqitch `camera_site_human_decisions` change widens the disposition/relation CHECKs and grants `sig_materialize` SELECT on `review_decision` — append-only, no UPDATE/DELETE; `infer_duplicate_targets` groups same-source records that are provably the same published row (row-identical, or byte-identical cited captures across targets + the same upstream ref) — the sole evidenced exception to constraint (a) — recorded as tier-0 `duplicate_target_of` auto-write edges while genuinely distinct same-source devices never merge; `camera_site_run.summary.human_review` + `duplicate_target_*` counts and the export's resolved-site metric count auto + human edges with pending proposals disclosed. BL-057 | P31.11 |
| ADR-117 | Presentation analytics emitted by the spine export (P31.14 / SURFACE.1, Round 9 — the design's ADR-R9-ANALYTICS; closes D-P27.5-1, advances D-P30.3-3 whose public half is P31.16): a new export-emitted `web/analytics/<name>.json` family — density_bins, centrality (+focus rule), decision_point, provenance (the three per-surface summaries), queue_meta — each carrying schema / as_of / a named denominator / `is_population_total: false` / its source compartments; density bins are deterministic H3 res-3 over tier-0 coordinate-reduced points only with distinct-source coverage (low coverage never reads as low density); centrality is undirected degree over the SAME typed access edges network.json emits (materialized when present, shaping fallback), focus = max degree then lexical id, `er_quality: null` with the inline no-ER-eval disclosure; the decision point derives the earliest SIG-UI-014b `next_decision_date` over the watch (never a stored field, `null` when none is derivable); the site tier distribution uses the §10.6 ordinal composition (`reconcile.weight.weight_class`, C derived per predicate half-life at as_of, D6 excluded, unweightable → `untiered`); files are licence-labelled like `web/map.json` — CC-BY → `web`, any other single or compound label → restricted `web_mixed` — with `assert_separated` re-proven; the web data seam reads them fail-loud (a missing analytic is a build error, never a demo constant or silent empty state) and `ops/publish.py` still refuses `web/presentation/`; `jurisdiction_dossiers` stays the only deliberately-demo presentation read. BL-056 | P31.14 |
| ADR-118 | Per-compartment z0–z14 vector tiles, repo-owned sig-web compression, and the retirement of the combined /map/points.json (P31.15 / SURFACE.2, Round 9 — the design's ADR-R9-TILES; advances D-P30.3-2 whose public half is P31.16; ends accepted deviation R8-1 in the build): every export `sites.geojson` is rendered to `web/tiles/<compartment>-sites.pmtiles` — one archive per licence compartment, never merged — carrying the z0–z14 pyramid, a `sites` MVT layer, `sig:license` and the compartment's attribution (ODbL keeps the OSM notice); the production renderer is pinned tippecanoe 2.79.0 (sha256 multi-stage build in the export image) with a deterministic pure-Python PMTiles v3 encoder as the CI/local fallback, both byte-deterministic (normalised metadata — no path/output-name/generator_options variance) and slimmed to the §19.4 publishable properties; the manifest registers each archive (the contract the web build reads); Q8 = no basemap — the style is a plain background plus one attributed `pmtiles://` source per compartment (the dead `osm_basemap` reference removed; a self-hosted extract is a costed follow-up only, a third-party tile service is out); Q9 = the combined licence-mixed `/map/points.json` is retired from the build + island — the island layers the per-compartment archives (ODbL 4.4(b) produced-work compositing) and an export-mode build with no archives fails loud; the first repo-owned sig-web image (`ops/web/Dockerfile`) compiles `google/ngx_brotli` + its pinned `deps/brotli` submodule as `--with-compat` modules against the exact nginx 1.27.5 source (Alpine's packaged module is ABI-incompatible) with `ops/web/nginx.conf` (gzip + Brotli text types, the registered `application/vnd.pmtiles` media type + byte ranges, never recompressed, immutable `/_astro/` only); `ops/gcp/web.sh` is the first repo-owned deploy path — the hand-made service spec read live and reproduced (gcsfuse `sig-web` bucket → `/mnt/sig-web`, unauthenticated ingress) on a `pin_image_digest` image; the roll + republish + hosted verification are P31.16's. BL-056 | P31.15 |
