...
| [`3_sig_golive_spec.md`](./3_sig_golive_spec.md) | **append-only** | The ratified go-live & productionization specification (rounds 3–4) that `decompose-spec mode=extend` instantiated into manifest rows 67–87. It scopes what remains between the composed-green fixture build and a live, public first jurisdiction, under the three-lane policy (human gates / return-pass re-runs / new contracts, GL-META-00). `GL-*` requirement ids are append-only; a landed requirement changes via a new version + an ADR, never a silent rewrite. Rounds are ratified independently — do not execute an unratified round. |
| [`adr/`](./adr/) | **append-only** | Architecture Decision Records (Appendix F). A decision is immutable; a change is a **new** ADR (SIG-ENG-003). The index [`adr/README.md`](./adr/README.md) is **generated** by `build-memory adr-index` (`scripts/adr-index.sh`) — never hand-edited; every new ADR adds an index row + an Appendix F row in the same PR (SIG-ENG-039). |
| [`build/`](./build/) | **historical record** | Durable **committed** build memory (build-memory v2, ADR-073): the machine-state ledger [`build/LEDGER.md`](./build/LEDGER.md), the per-ticket run ledgers [`build/runs/`](./build/runs/) and PR bodies [`build/pr/`](./build/pr/), the build index, coverage matrix, capstone/backlog/reconciliation reports (under [`build/reports/`](./build/reports/)), integration plan and release notes. Records of what happened; not rewritten. Validated by `scripts/docs/check-build-memory.sh`. The marker + index [`build/README.md`](./build/README.md) is living; `build/logs/` is gitignored. |
...
| [`tickets/DEFERRALS.md`](./tickets/DEFERRALS.md) | **append-only** | The deferred-obligations register (build-memory v2): one row per owed gate-pending obligation; read first every run, closed by the run it unblocks; cites `docs/build/BACKLOG.csv` `BL-` ids. Never overwritten or deleted. |
