<!--
  ...
  docs/3_sig_golive_spec.md (2026-09-09). tail=minimal per spec D6 + Appendix B item 7:
...
-->
# P24.8 — Go-live readiness delta (REC.1, reconcile)

- **Semantic id:** `REC.1` — renumbered from `86_REC.1` to `P24.8` for lexical chain order; the semantic id is unchanged.
- **Sequence:** P24.8 (was 86 of 87) · **Phase:** reconcile · **Kind:** reconcile
- **Tag:** golive-tail
- **base_branch:** current checkout
- **Depends on:** CCOPS.1 (row 85) — the last Round-4 unit
- **Run:** `implement-spec spec=docs/tickets/P24.8__golive-readiness-delta.md live_verification=false`
- **Gate status:** none
- **Live stage:** none
...
- `docs/3_sig_golive_spec.md` Part I §3 (D6); `docs/build/OPERATIONAL_READINESS.md`,
...
The go-live readiness ids of `docs/3_sig_golive_spec.md` (GL-* areas), delta scope.
