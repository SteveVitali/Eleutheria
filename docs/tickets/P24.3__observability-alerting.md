...
- **Semantic id:** `OBS.1` (**GL-OBS-01**) — renumbered from `81_OBS.1` to `P24.3` for lexical chain order; the semantic/GL ids are unchanged.
- **Sequence:** P24.3 (was 81 of 87) · **Phase:** 22+ (productionize) · **Kind:** ticket
- **Tag:** golive-round4
- **base_branch:** current checkout
- **Depends on:** DEPLOY.1 (a live stack to observe); SCHED.1 (scheduled runs to alert on)
- **Run:** `implement-spec spec=docs/tickets/P24.3__observability-alerting.md live_verification=false`
- **Gate status:** none
...
- `docs/3_sig_golive_spec.md` Part II § OBS.1 (GL-OBS-01); Part I §3 (D4 zero-cost).
