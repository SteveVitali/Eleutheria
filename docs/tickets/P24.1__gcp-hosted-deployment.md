...
- **Live stage:** operator-gated: GCP `apply` (GL-GATE-04 — project `$SIG_GCP_PROJECT`, zero/low-cost design)
...
- `docs/3_sig_golive_spec.md` Part II § DEPLOY.1 (GL-DEPLOY-01); §0.1 GL-GATE-04; Part I §3 (D4).
...
1. `ops/gcp/` infra-as-code (a Terraform module **or** idempotent `gcloud` scripts) parameterised
   by project id (`$SIG_GCP_PROJECT`) / region: **GCS** bucket(s) for the static site +
...
- Owns the Cloud-SQL-vs-GCE decision (ADR-`DEPLOY`). The real `apply` is a RETURN PASS action
  ...
  Provenance: GL-GATE-04 pre-answers the *host choice* (GCP `$SIG_GCP_PROJECT`); it does not
