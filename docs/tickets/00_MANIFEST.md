...
| 79 | `P24.1__gcp-hosted-deployment.md` | 22+ | ticket | **C** | **[semantic `DEPLOY.1`] DEPLOY.1 (GL-DEPLOY-01):** `ops/gcp/` infra-as-code (GCS + Cloud Run + Cloud SQL/`e2-micro`) on GCP `zeta-medley-508121-u7`; `sig-ops deploy --target gcp`; backups + tested restore drill. | HG-12 — IaC written+validated; real `apply` gated on operator `gcloud` ADC |
