...
| 88 | `P25.1__live-fetch-operationalization.md` | 25 | ticket | **C** | **[semantic `LIVE-OPS.1`] P25+ BACKLOG (drafted 2026-09-15; NOT part of the DONE go-live chain, rows 47–87):** live-fetch operationalization — the crawler-conduct-vs-API **endpoint/robots policy decision** (a real `osm_overpass` live fetch is refused because `overpass-api.de` robots.txt disallows `/api/`, SIG-INGEST-037) + **document-connector content-drift hardening**; wire real `data/live_targets.toml` rows + run real fetches into the hosted spine (ADR-082; BL-055; advances DEFERRALS `D-P21.3-1`/`D-LIVE.1a-1`). | HG-03 + endpoint/robots policy |
