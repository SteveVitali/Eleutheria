...
| RISK-P17-03 → BL-043 → retired by P21.9 | The populated pathways are **instance graphs in the conformance suite**, not rows persisted to the claim spine | P17.1 is the §5.2 expressibility proof, not an ingestion connector; live population arrives with the Stage-5 connectors (P17.2/P17.3) over the same frozen schema | The instance shapes mirror the generated model exactly (they *are* the generated Pydantic classes); persisting them is additive and needs no schema change, which is precisely what this ticket proves. |
...
| RISK-P21-12 | **A MapRoulette challenge could leak a sensitive coordinate.** A contribution-back challenge pins a mapper to a specific device location; if a task about a confidential facility (DV shelter, protective, undercover — C4), a private-residence candidate (C3), or a mobile asset (C5) were included in a push payload, SIG would publish a coordinate its own sensitivity matrix forbids — a Part VIII §0.7 / §43.3 breach, and irreversible once pushed to a public challenge. | **Compensating control:** the push is **tier-filtered before the payload exists** (P21.7, ADR-069). `tasks.maproulette.build_challenge_payload` drops every task whose sensitivity class publishes no geometry (`policy.sensitivity.geo_tier_for(cls) >= SENSITIVE_GEO_TIER_FLOOR` = 3, i.e. C3/C4/C5); excluded tasks contribute only to `excluded_sensitive_task_count`, never a coordinate. `tests/tasks/test_tasks_maproulette.py::test_sensitive_tier_task_is_excluded_from_push_payload` and the CLI test (`excluded_sensitive_task_count == 1`) pin it. Only C1 (exact) and C2 (reduced-precision) tasks — publicly-visible hardware — are ever pushed. |
| RISK-P21-13 | **Hashtag squatting could pollute or spoof the §7 metric.** The leverage metric counts public changesets bearing `#sig_operator_attribution`; a third party could add the hashtag to unrelated or malicious changesets, inflating or discrediting the count, or a look-alike challenge could impersonate SIG's activity. | **Compensating control:** the hashtag is cross-checked against SIG's **registered Organised Editing activity page** and its declared challenge id (`ops/config.toml [tasks.contribution] oe_page` / `maproulette_challenge`); the activity page (`docs/governance/organised-editing-activity.md`) publicly names the account (`SIG_operator_attribution`) and the challenge so any observer can distinguish a SIG-originated challenge from a squatter. The metric is deliberately **auditable, not authoritative**: it is a lower-bound count of *publicly verifiable* hashtag-bearing changesets, stated as a task outcome (SIG-CONTRIB-016g), never a headline SIG cannot defend. Attribution stores only the changeset id (Part VIII §0.7), so a spoofed changeset is a public, inspectable id — not hidden state. Deeper provenance cross-checking (challenge-id match on each changeset) is a refinement enabled once a live challenge id is recorded (HG-08). |

## Phase 21 — Operationalization (P21.8)

Per §53 / SIG-ENG-031, P21.8's risk-register entries. P21.8 lands the first-class
`data_driven` connector for EFF/MuckRock's Data Driven releases (§23.9) and the
coarse-international REFERENCE-capture path (§47), both over committed fixtures behind
the loader gate (HG-03 pending — no source flipped). Part VIII §0.7 is binding: Data
Driven is per-agency **aggregate rows only**, never per-search/per-person/per-edge.

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P21-14 | **A Data Driven release schema could change between versions and be silently mis-parsed.** The corpus is re-released over time (a 2018 release, a corrected re-release, a companion single-state file), and its columns — vendor-specific retention windows in incommensurable units, sharing degree, scan/hit aggregates — could shift shape between versions; a parser that assumed one shape could silently drop or mis-map a column, corrupting a historical baseline that later vendor-replacement analysis (§29.4) is measured against. | **Compensating control:** the release is the **unit of ingestion** and is **pinned by a content digest per version** (`connectors.data_driven.ReleaseManifest.digest`, SIG-INGEST-043b): every claim carries `release_version` + `release_digest` + `retrieved_date`, so a re-ingest under a new version is a **new dated claim set that never overwrites** the prior one (SIG-INGEST-043d/017) — the old interpretation is retained, not corrupted. `tests/connectors/test_data_driven.py::test_versioned_reingest_appends_new_dated_claims_and_overwrites_nothing` and `::test_release_digest_changes_between_versions` pin it; per-column retention windows are preserved with their unit and never normalized together (`::test_retention_window_preserved_per_column_never_normalized`). No live fetch runs this ticket (HG-03 pending); a future schema drift surfaces as a new digest + a shadow diff for review, never a silent overwrite. |
| RISK-P21-15 | **A per-search / per-person / per-edge column could enter the graph.** Data Driven is per-agency aggregate data, but an upstream file could carry a per-search row, a per-plate read, an officer name, or a sharing **edge list**; ingesting any of these would enable the de-pseudonymisation join §43.2a forbids (RISK-P0-08) and manufacture the unexplained network edge the defining standard forbids (SIG-INGEST-043c) — an irreversible Part VIII §0.7 breach. | **Compensating control:** the connector enforces **aggregate-only at the ingest boundary**: `connectors.data_driven.assert_aggregate_only` scans the release's declared columns **and** every agency row's own keys against a data-defined forbidden-token list (per_search/per_plate/per_person/officer/sharing_edge/edge_list/partner_list…) and **refuses the whole release** with `PerSearchColumnError` before a single claim is built; the predicate allowlist is a second hard schema gate (`assert_predicate_allowed`). Sharing is recorded as **degree only, never the edge list** (`sharing_partner_degree`, `degree_only_no_edge_list=True`). `tests/connectors/test_data_driven.py::test_per_search_column_release_is_refused`, `::test_assert_aggregate_only_rejects_forbidden_tokens`, and `::test_sharing_is_degree_only_never_an_edge_list` pin the rejection. |
| RISK-P21-16 | **The pathway connectors could over-infer a deployment from a procurement.** A procurement record proves a technology was *purchased*, not that it is *operating*; a Stage-5 pathway connector that read a contract and asserted a `deployed` claim would manufacture an operational fact the evidence never supported — the defining-standard §3.1 breach (synthetic certainty) at the exact seam §46 warns about (`procured` ≠ `deployed`). Left unguarded, every itemised purchase order would silently become a live-deployment claim. | **Compensating control:** the `pathways` connector types every claim (`vendor_product`/`procurement`/`policy`/`deployment`) and enforces `procured` ≠ `deployed` with **two guards inside `pathway_claim`** so neither can be bypassed: a **genre-level** guard (`assert_claim_type_supported_by_genre` raises `DeploymentInferenceError` unless the document genre ∈ `parsing.genre.DEPLOYMENT_GENRES = {deployment_report}`) and a **predicate-level** guard (`assert_deployment_predicate_has_deployment_type` — `deployed`/`actually_provides_capability`/`in_operation` may ride only a `deployment` claim). The genre is **re-derived from the document text** (`parsing.genre.classify_genre`), never trusted from a fixture label, so a procurement document cannot be relabelled to unlock a deployment claim. `tests/connectors/test_pathways.py::test_deployment_claim_from_procurement_genre_is_refused`, `::test_deployment_predicate_on_a_non_deployment_claim_is_refused`, `::test_mislabelled_genre_is_rejected_genre_is_re_derived_from_text`, and `tests/connectors/test_pathway_coverage.py::test_no_deployment_claim_from_a_procurement_genre_document` pin it; `parsing.genre` classification is unit-tested (`tests/parsing/test_genre.py`). |
| RISK-P21-17 | **The ADR-033-deferred parser layers could regress on real PDFs.** The layer-4 table extraction and layer-3 clause locators (`parsing.tables`/`parsing.clauses`) run over committed text fixtures; an upstream PDF whose real layout differs (merged cells, reordered columns, an unheaded clause) could be mis-parsed, silently corrupting a vendor/product/amount cell or dropping a policy fact — the §24.3 parser-drift risk R11 names a top-5 operational hazard. | **Compensating control:** the layers follow the SIG-PARSE-007 posture — a **committed fixture corpus** (`tests/connectors/fixtures/pathways/*.json`, `tests/parsing/test_tables.py`/`test_clauses.py`) pins known inputs to expected outputs, and a **`shadow` diff** over the fixtures is byte-identical on re-run (0 diffs, `tests/connectors/test_pathways.py::test_shadow_replay_over_fixture_has_zero_diffs`), so a careless edit fails a test rather than quietly changing what SIG extracts. A ragged table row is **padded, never dropped** and an unparseable cell keeps its raw literal (SIG-PARSE-004); a headingless policy document yields a `whole` clause so a fact is never dropped for want of a locator. The heavy real-PDF/OCR engines remain deferred behind the same frozen layer vocabulary (ADR-033), so adding one is a new engine behind the same contract, not a rewrite. |

## Phase 22 — Documentation (P22.1)

Per §53 / SIG-ENG-031, P22.1's risk-register entries. P22.1 is the first whole-repo refresh of the
human-facing documentation against the finished code (the `refresh-repo-docs` skill, `audit: full`):
READMEs, `CONTRIBUTING.md`, `CHANGELOG.md`, the new `docs/README.md`, and the governance/build/adr
index READMEs. The report-only set (generated spec, frozen research, append-only registers,
historical build/ticket/slice records, ADR bodies) is audited but never edited. The only non-doc
additions are the vendored detector (`scripts/docs/check-repo-docs-freshness.sh`) and the
`make docs-check-repo` target; no product code changed.

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P22-01 | **Docs drift resumes after this pass.** A one-off refresh converges the docs onto the code at HEAD, but every later change to a command, path, connector count or run mode can silently re-introduce drift — the failure mode `refresh-repo-docs` names, where a reader follows a stale setup step and loses trust in every other doc (SIG-ENG-001, the repo is executable from its documents). | **Compensating control:** the deterministic detector is **vendored into the repo** (`scripts/docs/check-repo-docs-freshness.sh`, MIT) and exposed as `make docs-check-repo`, so drift is checkable without the skill installed; it exits non-zero on any broken reference over the in-scope corpus (0 critical at this ticket's close). **P22.2 wires both docs detectors into CI** so drift is caught at change time, not by a periodic sweep, and `INTEGRATION_PLAN.md` §(d) makes a docs refresh part of every release cut. The check is fast and read-only. |
| RISK-P22-02 | **A "fix" to a historical or generated record would falsify evidence.** The refresh touches many files near append-only registers, the generated spec, frozen research, ADR bodies and executed ticket/build records; editing any of those to "make a doc match" would rewrite evidence (a P1–P3 / §3.1 breach) — e.g. inventing a CHANGELOG entry, or "correcting" a frozen research finding, or editing `docs/2_canonical_design_spec.md` directly instead of via `spec_src` + an ADR (SIG-ENG-003). | **Compensating control:** the ticket's **scope boundary is explicit and mechanically checkable** — the in-scope doc set vs the report-only/never-edit set is enumerated, and `git diff --name-only <base>..HEAD` is asserted to contain no report-only path (spec, `docs/research/**`, ADR-*.md bodies, registers other than the appended P22.x sections, `docs/build/*.md` other than `README.md`/`DOCS_REFRESH_REPORT.md`/`BACKLOG.csv`, `docs/tickets/**`). Findings inside the report-only set are recorded in `DOCS_REFRESH_REPORT.md §4 (excluded-scope findings)` with an owner, never fixed here; the CHANGELOG is backfilled **only** from `git log`/PR evidence and `0.1.0` stays "unreleased"; the generated spec is regenerated via `BUILD.sh` (verified byte-clean), never hand-edited. Checked in review. |

## Phase 22 — Documentation (P22.2)

Per §53 / SIG-ENG-031, P22.2's risk-register entries. P22.2 converges the agent-facing `AGENTS.md`
hierarchy on the finished code (the `agent-docs` skill, `mode=refresh deep=true` — NOT clean-slate:
P19.1's docs are kept, uncovered packages bootstrapped, drifted references regenerated, hand-written
gotchas preserved), adds a `CLAUDE.md` bridge, vendors the agent-docs detector
(`scripts/docs/check-agent-docs-freshness.sh`), and wires **both** doc detectors into CI via
`make docs-check`. No product code changed.

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P22-03 | **Agent docs exceed the harness budgets and stop being read.** Over-long `AGENTS.md` files are silently truncated (OpenAI Codex concatenates the chain root-down under a 32 KiB cap) or reduce adherence (Claude Code guidance: files beyond ~200 lines "consume more context and reduce adherence"); a bloated root doc that an agent never fully loads is worse than a short accurate one, and defeats the whole ticket. | **Compensating control:** the length budgets are **asserted by the vendored detector**, not left to taste — a leaf/package `AGENTS.md` stays ~150 lines, the root ~250 (root is 161 lines; every package doc ≤70). `make docs-check` (in CI on PRs) fails on the detector's structural issues, and the guideline order (`Purpose → … → Do → Don't`) keeps each file scannable. Content is kept factual (every statement verified against a file or a run, §3.1), so lines earn their place instead of padding the budget. |
| RISK-P22-04 | **A contradiction between the root and a package `AGENTS.md` makes agents pick a rule arbitrarily.** With nested docs, two files can state a build command or a boundary differently; an agent reading the nearer file would follow a rule the root contradicts, producing inconsistent behaviour across packages — the exact failure the AGENTS.md standard's precedence rule exists to prevent. | **Compensating control:** **nearest-file-wins is stated explicitly** in the root and every package doc (the package file adds to and wins over the root on conflict), and the skill's **consistency sweep** re-reads every touched doc plus its parent to resolve conflicting commands/conventions before commit. Package docs scope to package-specific rules (e.g. `web/` zero-JS, `db/` sqitch/append-only, `connectors/` the ingestion gate) rather than restating — and possibly contradicting — the root. |

## Capstone verification — `sig-postbuild` (composed end-to-end)

Final composed verification (`docs/build/CAPSTONE_VERIFICATION.md`): two integrity gaps closed
(MATRIX-INT-01, APPENDIX-F-01), then the whole build run as one unit with Docker up. Append-only; the
signed `CAPSTONE_CLOSURE.md §(b)` table and ADR bodies are unchanged (addenda only).

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P20-01 (materialized) | **Backlog drift went uncaught at the chain tip.** `check_backlog.py` is a PR-time docs tool, **not** a `make check` step, so `docs/build/BACKLOG.csv` accumulated enum/mapping drift (`BL-041` landing `P21.x`; `BL-043` status carries inline prose; `BL-052` in no theme; `ADR-071`/`ADR-072` unmapped; orphan + double-owned sources) that a green `make check` never surfaced. | **Compensating control:** the capstone verification **runs `check_backlog.py` explicitly** and records the full failure set + the closing command in `CAPSTONE_VERIFICATION.md §(f)`; the fix is routed to the operator as a scoped `P22+` backlog-reconciliation ticket (reconcile `BACKLOG.csv` + `BACKLOG_THEMES.md` against the risk register / ADR revisit triggers / `LEDGER_DEFERRALS.md` until the checker exits 0). This run left the file byte-identical to the tip — the RED is reported, never fabricated green. |
| RISK-PVER-01 | **A composed "green" that silently skipped the Docker/web seams would be a false pass.** `tests/db`/`tests/e2e` skip without Docker, and S8 (export→web, LD-V08) skips without `web/node_modules` — a Docker-less or Node-less run could look green while proving nothing. | **Compensating control:** the capstone ran with **`SIG_REQUIRE_DB_TESTS=1`** (missing daemon fails loud) over a real PG18+PostGIS container, and with `web/node_modules` present so **S8 RAN** (verified `-v`: the three `test_s8_*` cases PASSED, LD-V08 rendered from export bytes) — never skipped. `run_okc.sh` additionally drove the live spine→export→web→acceptance path (299-vs-190 from the export, J-1 pass, 0 failed). Evidence + counts recorded in `CAPSTONE_VERIFICATION.md §(b)`. |

## Phase 22 — Build memory v2 (P22.3)

Build-memory v2 migration (ADR-073): retire `.agents/scratch/`, commit the build's memory under
`docs/build/` in the v2 layout, seed `docs/tickets/DEFERRALS.md`, regenerate the ADR index, and vendor
`check-build-memory.sh` into `make docs-check`. Move/rename only; no product code changed.

| Risk | Description | Compensating control |
|---|---|---|
| RISK-P22-05 | **Committing the run ledgers / machine ledger / PR bodies under `docs/build/` could expose a private individual's personal data** (the ledgers narrate a surveillance-graph build that names officials, vendors, and sources). Once committed, a leak is permanent (append-only, P1–P3). | **Compensating control:** a Part VIII personal-data scan was run over **every newly-committed file** before staging (the migrated `LEDGER.md` + `runs/*` + `pr/*` + `reports/*`, the DEFERRALS/ADR/manifest/docs edits). Result recorded in the PR body and `docs/build/runs/P22.3.md`: only **public officials, public agencies/vendors, public source URLs, and gate ids (HG-nn)** appear — **no private individual's plate/trip/per-person data, no home address, no non-public PII**. The validator's secret grep (`AKIA…`, `sk-…`, `ghp_…`, PEM keys, `xox…`) is green; ledgers record credentials as `provided: yes/no`, never values. Re-runnable: `bash scripts/docs/check-build-memory.sh .` (secret scan) + the documented grep set. |
| RISK-P22-06 | **The generated ADR index (`docs/adr/README.md`) can silently drift from the `docs/adr/ADR-*.md` files** once it is machine-generated rather than hand-maintained — a stale index misleads readers and breaks the Appendix F ↔ files contract (SIG-ENG-039). | **Compensating control:** the index is generated by `build-memory adr-index` (`scripts/docs/adr-index.sh`) and **validated in `make docs-check`**: the vendored `scripts/docs/check-build-memory.sh` regenerates the index to a temp file and diffs it against the committed `docs/adr/README.md`, failing on any hand edit or drift; `docs/build/tools/check_spec_src.py` additionally asserts the Appendix F table equals the `docs/adr/` file set (72 ADRs after ADR-073). Both are exit-code-gated and CI-runnable. |

## Phase 24 — Productionize (P24.2 — re-ingest cadence, SCHED.1 / GL-SCHED-01)

Re-ingest cadence & orchestration (ADR-076): a minimal GitHub Actions cron drives
`sig-connectors run` per due source on a per-source interval; freshness is an append-only
JSONL ledger; disappearance rides the volatility-proportional sweep cadence. Builds on the
P21.3 `PoliteFetcher`/runner and the existing disappearance classifier — no duplication.

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P24-01 | **The re-ingest scheduler could poll a source faster than its etiquette tolerates.** A cadence that fires too often — hammering a small civic host or a public Overpass instance, re-running before the previous fetch settled, or ignoring a declared refresh window — would burden the source, trip its rate limiter, or get SIG blocked, violating crawler conduct (§26 Rule 3, SIG-INGEST-011/037) and Overpass etiquette (SIG-INGEST-045d/045h). This is RISK-P21-04 raised one layer up: not the per-request pace, but *how often a run happens at all*. | **Compensating control:** cadence is a **two-layer** design (ADR-076). *Coarse:* `orchestration.cadence.reingest_interval_days` derives a per-source interval from the registry `cadence` field, defaults to **30 days**, and is **clamped to a `MIN_INTERVAL_DAYS = 1` etiquette floor** — the scheduler never drives a source faster than daily even if the text says "hourly"; `sig-orchestration due` runs a source only once its interval has elapsed since the last recorded ingest (the append-only `FreshnessLedger`). *Fine:* every `sig-connectors run` the cron invokes egresses through the shared `PoliteFetcher`/`HttpxTransport` (per-host crawl-delay, retry-with-backoff on 429/503/504 honouring `Retry-After`, Overpass 429/504 = back off), which the scheduler never bypasses; and a non-green source is **refused (exit 3) before any socket opens** (HG-03), so no source is contacted until an operator flips it. The interval mapping + the floor + the not-due skip are asserted deterministically (`tests/orchestration/test_reingest_cadence.py`). |

## Phase 24 — Productionize (P24.3 — observability & alerting, OBS.1 / GL-OBS-01)

Observability & alerting (ADR-077): record-first/notify-second — every alert appends to an
append-only recorded-alert JSONL ledger before any external delivery; the notifier seam is a
always-on `SIG-ALERT` log line + an env-configured webhook (HG-09). Metrics are a bounded JSONL
probe log; uptime/error budgets are a pure function over the rolling window; retention is an
age + byte cap. Consumes INFRA.1's `ops.egress` alarm — does not re-implement it.

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P24-02 | **The alerting path could leak an env secret into a log, the ledger, or the wire — or a breach could fire nothing at all if recording depended on delivery.** A webhook URL commonly embeds its token in the path; if an alert's message/detail (or a notifier error) echoed it, the secret lands in a durable artifact (append-only — a leak is permanent, P1–P3). The mirror failure: a notifier that *notifies only* loses the alert entirely when the endpoint is down or unconfigured — a silent alerting pipeline is worse than none (RISK-P0-12's "fails silently is not a plan"). | **Compensating control:** *record-first, notify-second* — `ops.alerts.fire` appends to the recorded-alert ledger **before** any external delivery, so an unconfigured/failed notifier never loses the alert (the deterministic artifact stands alone). *Structural scrubbing* — `scrub_secrets` masks the **values** of secret-looking env vars (`*_TOKEN`/`_SECRET`/`_PASSWORD`/`_KEY`, plus `SIG_ALERT_WEBHOOK_URL` itself) in every emitted surface: the ledger row, the `SIG-ALERT` log line, and the webhook body; the URL is never logged. Both halves are asserted by tests that fail if removed (`tests/ops/test_alerts.py`: egress breach + keepalive failure each record an alert; no env secret appears in the ledger or captured logs; the webhook POST body is scrubbed while the Bearer header is used). Offline honesty holds: unprobed services report `no-data`, never a fabricated 100% (§3.1). |

## Phase 24 — Productionize (P24.4 — CI composed hardening, CI.1 / GL-CI-01)

CI hardening (ADR-078): a dedicated `composed` CI job runs `tests/e2e` for real with Node +
Docker present; a nightly cron re-runs it plus the three scans; secret/licence gates are
vendored stdlib scanners; the dependency audit is pip-audit→OSV (nightly, not PR).

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P24-03 | **A scanning gate could silently no-op — a scanner that errors to "clean", a workflow step that stops running, or a network-dependent audit that reports green when the advisory DB was never reached — while the "CI runs the composed suite" claim rests on a job nobody can see.** The dangerous failure of a gate is not a red run but a *green-looking* non-run (RISK-P0-12's "fails silently is not a plan" applied to CI itself). | **Compensating control:** every new behaviour is pinned by a test that fails if it is removed — `tests/unit/test_ci_workflows.py` asserts the `composed`/`security`/`docs` jobs and the nightly schedule + report/upload steps exist and carry the right commands (`SIG_REQUIRE_DB_TESTS=1` so a missing Docker daemon is a hard failure, never a silent skip); `tests/unit/test_security_scanners.py` proves the scanners *catch* seeded violations (a planted AWS key / PEM / `SIG_*` literal → exit 1; BUSL / unresolvable / new-GPL licence records → exit 1) and that the secret scanner never echoes the matched text into logs. The nightly report step **is** the gate (`nightly_report.py` exits non-zero on any failed stage, and the artifact uploads `if: always()` so a red nightly still carries its evidence). The dep audit's network dependence is honest: it rides the nightly (where a transient failure reads as a red night, investigated) rather than the PR gate; and its first real run *did* catch live CVEs (starlette 0.48.0 → fixed by the fastapi/starlette bump in this same change). |

## Phase 24 — Productionize (P24.6 — second jurisdiction, JURIS.2 / GL-JURIS-01)

France (Commune de Gex, Ain — vidéoprotection) through the P18.1 adapter
framework (ADR-079): a `_SEED_SLICES` dispatch in `ops.seed`, the four
`france_belgium` source→connector mappings, the France export request +
FR-GDPR dossier, France's own acceptance module, and `run_france.sh`.

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P24-04 | **The jurisdiction filter is a substring convention, not a structure — and the second jurisdiction could quietly produce an *empty* spine slice or a colliding one.** `reconcile resolve --jurisdiction <j>` / `sig-resolution match` filter on `entity_identifier.value ILIKE '%<j>%'`; the only thing connecting a claim to "france" is the subject string carrying the token. A jurisdiction slug that collides with non-jurisdiction identifier text (or a slice whose subjects forget the token) resolves the *wrong* claims — or silently resolves *none*, which reads as a green run. | **Compensating control:** the France slice's subjects are pinned to carry `france` by a test that fails on any subject missing it (`tests/ops/test_seed_france.py::test_france_slice_subjects_carry_the_jurisdiction_token`); the material-fact predicates are pinned resolver-registered + genre-admissible (`test_france_slice_uses_resolver_registered_predicates_for_material_facts`, `test_france_slice_claims_use_directness_genres`) so a claim the resolver drops can't pass silently; and the acceptance module asserts resolved *values* (FR-1..FR-6 expect `authorized`, `Code de la sécurité intérieure…`, `50754`, …), not just 200s — an empty/mis-resolved slice fails FR queries loudly. The DECP-rights gap is *also* asserted (`test_france_export_excludes_decp_content` + the dossier's `UNRESOLVED` row) so the §42 fail-closed posture can't be bypassed by accident. |

## Phase 24 — Productionize (P24.7 — CCOPS disclosure connector, CCOPS.1 / GL-CCOPS-01)

The `government_mandated_disclosure` connector class (ADR-080): municipal
surveillance-ordinance disclosures → per-agency aggregate claims, one connector
with three per-source extractors (Seattle SIR, NYC POST, SF inventory).

| id | Risk (what breaks the acceptance gate if unhandled) | Compensating control |
|---|---|---|
| RISK-P24-05 | **A mandated disclosure could over-assert use — authorization read as deployment — or a blank mandated field could be read as an answer.** CCOPS documents are epistemically mixed: an SIR authorizes and constrains (pre-acquisition commitment), an inventory row's `approved` status is procurement, and only an `in_use` status is the compelled statement of use. Conflating them plants synthetic deployment claims; conflating `present_but_empty` with `answered` plants fabricated values (e.g. a zero cost where the city left the field blank). | **Compensating control:** two mechanical layers — the use predicates may ride only on `disclosure_use` claims, and `disclosure_use`/`compliance_finding` claims require a document that itself reports use (deployment genre, or `reports_actual_use` — never a procurement/vendor genre however labelled), with the genre re-derived from the text not the fixture label. Tests pin every edge: the Seattle/NYC fixtures emit zero use claims; SF emits `in_use` for exactly the five in-use-status rows and none for approved/seeking rows; a mislabelled fixture is rejected; a procurement document carrying `reports_actual_use` still refuses. The field-state vocabulary (`answered`/`present_but_empty`/`absent`) is pinned by tests asserting the blank Fiscal rows land as `disclosure_field_state` claims and never fabricate `acquisition_cost`/`operating_cost` values. The aggregate schema gate (missing `agency`/`reporting_period`/`technology`, forbidden columns, forbidden tokens, person-naming) is asserted by four refusal tests plus a sweep over every emitted row at `normalize`. |
