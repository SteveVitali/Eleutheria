...
| id | Requirement | Why not fully closed now | Compensating control |
|---|---|---|---|
| RISK-P7-05 | The **concrete extraction engines** for layers 3–5 (PDF text/table, OCR). Only the layer *selection* and interface are built; classification routes to a layer, it does not run one. | The heavy libraries (and OCR) are the job of the P07.2/P07.3 connectors that extract through this interface; wiring them here would pre-empt those tickets and add runtime dependencies with no caller. | The interface is complete and tested end-to-end; the scanned-PDF signal is a deterministic byte heuristic whose mis-route is corrected downstream, never a silent drop (ADR-033 revisit trigger). |
| RISK-P7-06 | The **nightly canary schedule** (SIG-PARSE-008). The deterministic drift core exists; the scheduled fetch-a-live-sample-and-alert job does not. | The ops schedule and alert destinations are an operations concern (mirrors the P05.2 gold-set cadence, RISK-P5-11), not a code ticket. | `structural_findings`/`run_canary` are pure and tested; the nightly job is a thin fetch-and-call wrapper whose alerting contract is a tracked ops deliverable. |
