...
# Copyright (C) 2026 The SIG project (vendoring) and the refresh-repo-docs skill authors.
#
# Vendored, unmodified-in-logic copy of the refresh-repo-docs drift detector so
# this repository can run the human-facing-docs freshness check WITHOUT the skill
# installed (e.g. in CI, deliverable P22.1/P22.2).
#
# Provenance: copied from ~/agent-skills/skills/refresh-repo-docs/scripts/
#   check-repo-docs-freshness.sh on 2026-09-09 by ticket P22.1. The MIT licence
#   of the source skill is retained above. The ONLY change from the source is the
#   corpus-discovery `find` filter below, which adds this repo's scope boundary
#   (P22.1 §"Scope boundary"): virtualenv/third-party trees and the report-only
#   doc set (frozen spec/research, append-only registers, historical build/ticket
#   records, ADR bodies) are excluded so the detector audits exactly the in-scope
#   human-facing docs. The detection logic (broken refs, stale suspects, JSON out,
#   exit codes) is byte-for-byte the upstream skill's.
# ---------------------------------------------------------------------------
...
# The corpus: *.md/*.rst/*.adoc, minus agent docs and harness dirs (upstream),
# minus this repo's virtualenv/third-party trees and its P22.1 report-only set.
DOCS="$(find "$SCOPE" -type f \( -name '*.md' -o -name '*.rst' -o -name '*.adoc' \) \
  ...
  -not -path '*/.venv/*' -not -path '*/venv/*' -not -path '*/.direnv/*' \
  -not -path '*/site-packages/*' -not -path '*/.pytest_cache/*' \
  -not -path '*/docs/research/*' -not -path '*/docs/tickets/*' \
  -not -name '2_canonical_design_spec.md' -not -name '1_deep_research_overview.md' \
  -not -name 'risk_register.md' -not -name 'traceability.md' \
  -not -name 'ADR-[0-9]*.md' \
