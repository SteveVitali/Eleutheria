...
# Copyright (C) 2026 The SIG project (vendoring) and the agent-docs skill authors.
#
# Vendored, unmodified copy of the agent-docs drift detector so this repository
# can run the agent-facing-docs freshness check WITHOUT the skill installed
# (e.g. in CI, deliverable P22.2), alongside P22.1's check-repo-docs-freshness.sh.
#
# Provenance: copied byte-for-byte from
#   ~/agent-skills/skills/agent-docs/scripts/check-agent-docs-freshness.sh
#   on 2026-09-09 by ticket P22.2. The MIT licence of the source skill is
#   retained above. The detection logic (file-reference / key-file / coverage-gap
#   / line-count checks, JSON report, exit codes) is unchanged from upstream.
# ---------------------------------------------------------------------------
